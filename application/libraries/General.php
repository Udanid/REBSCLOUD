...
class General {
	...
	function check_account($account_name)
	{
		...
		{
			$CI->messages->add('Account ac_settings file is missing.', 'error');
			return FALSE;
		}
		...
		if ( ! $account_data)
		{
			$CI->messages->add('Invalid account ac_settings.', 'error');
			return FALSE;
		}
		...
		{
			$CI->messages->add('Hostname missing from account ac_settings file.', 'error');
			return FALSE;
		}
	...
	function check_database()
	{
		...
		if ($CI->db->conn_id)
		{
			...
			if ($CI->db->query("SHOW TABLES"))
			{
				...
				{
					/* check for valid ac_settings table */
					$valid_ac_settings_q = mysql_query('DESC ac_settings');
					if ( ! $valid_ac_settings_q)
					{
						$CI->messages->add('Invalid account database. Table "ac_settings" missing.', 'error');
						return FALSE;
					}
					$this->check_database_version();

					$table_names = array('ac_groups', 'ac_ledgers', 'ac_entry_types', 'ac_entries', 'ac_entry_items', 'ac_tags', 'ac_logs', 'ac_settings');
				...
				$CI->messages->add('Invalid database connection ac_settings. Check whether the provided database name, username and password are valid.', 'error');
				return FALSE;
			}
		} else {
	...
	/* Check config/ac_settings/general.ini file */
	function check_setting()
	{
		$CI =& get_instance();

		$setting_ini_file = $CI->config->item('config_path') . "ac_settings/general.ini";

		/* Set default values */
		$CI->config->set_item('row_count', "20");
		$CI->config->set_item('log', "1");

		/* Check if general application ac_settings ini file exists */
		if (get_file_info($setting_ini_file))
		{
			/* Parsing general application ac_settings ini file */
	...
	function check_database_version()
	{
		...
		$CI->db->from('ac_settings')->where('id', 1)->limit(1);
		$account_q = $CI->db->get();
	...
	function setup_ac_entry_types()
	{
		//echo "sss";
		$CI =& get_instance();

		$CI->db->from('ac_entry_types')->order_by('id', 'asc');
		$ac_entry_types = $CI->db->get();
		if ($ac_entry_types->num_rows() < 1)
		{
			$CI->messages->add('You need to create a entry type before you can create any ac_entries.', 'error');
		}
		$entry_type_config = array();
		foreach ($ac_entry_types->result() as $id => $row)
		{
		...
		$CI->config->set_item('account_ac_entry_types', $entry_type_config);
	}
}
