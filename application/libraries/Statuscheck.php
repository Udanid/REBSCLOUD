...
class Statuscheck {
	...
	function check_permissions()
	{
		...
		/* Writable check */
		$check_path = $CI->config->item('config_path') . "ac_settings/";
		if (! is_writable($check_path))
		{
			$this->error_messages[] = 'Application ac_settings directory "' . $check_path . '" is not writable. You will not able to edit any application related ac_settings.';
		}

		$check_path = $CI->config->item('config_path') . "accounts/";
		if (! is_writable($check_path))
		{
			$this->error_messages[] = 'Account ac_settings directory "' . $check_path . '" is not writable. You will not able to add or edit any account related ac_settings.';
		}
		...
		if (substr(symbolic_permissions(fileperms($check_path)), -3, 1) == "r")
		{
			$this->error_messages[] = 'Security Risk ! The application ac_settings directory "' . $check_path . '" is world readable.';
		}
		if (substr(symbolic_permissions(fileperms($check_path)), -2, 1) == "W")
		{
			$this->error_messages[] = 'Security Risk ! The application ac_settings directory "' . $check_path . '" is world writeable.';
		}
