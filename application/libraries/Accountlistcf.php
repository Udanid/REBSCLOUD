...
class Accountlistcf
{
	...
	function add_sub_ac_groups()
	{
		...
		{
			$this->children_ac_groups[$counter] = new Accountlistcf();
	...
	function add_sub_ac_ledgers()
	{
		...
		$CI->db->select('ac_ledgers.*,ac_groups.name AS gname,ac_groups.parent_id')->from('ac_ledgers')->join('ac_groups','ac_groups.id=ac_ledgers.group_id')->where('ac_ledgers.group_id', $this->id)->where('ac_ledgers.active', '1');
		...
		foreach ($child_ledger_q->result() as $row)
		{
			...
			$this->children_ac_ledgers[$counter]['parent_id'] = $row->parent_id;
			$this->children_ac_ledgers[$counter]['gname'] = $row->gname;
			$this->children_ac_ledgers[$counter]['total'] = $CI->Ledger_model->get_ledger_balance_period_without_op($row->id,$this->sttime,$this->endtime);
	...
	function account_st_short($c = 0,$level = '',$color='')
	{
		$this->counter = $c;
		/*if ($this->id != 0)
		{
		...
		if (count($this->children_ac_ledgers) > 0)
		{
			...
			foreach ($this->children_ac_ledgers as $id => $data)
			{
				if($level=='L4'){
					...
					echo "<td class=\"td-ledger\" style=\"background-color:".$color."\">";
					echo $this->print_space($this->counter).$this->print_space($this->counter);
					...
					echo $data['name'];
					echo "</td>";
					//echo "<td align=\"right\" style=\"background-color:".$color."\">" . convert_amount_dc2($data['total']) . $this->print_space($this->counter) . "</td>";
					if($data['optype'] == 'D'){
						if($data['total'] > 0){
							echo "<td align=\"right\" style=\"background-color:".$color."\">" . convert_amount_dc2($data['total'] * -1) . $this->print_space($this->counter) . "</td>";
						}else{
							echo "<td align=\"right\" style=\"background-color:".$color."\">" . convert_amount_dc2(abs($data['total'])) . $this->print_space($this->counter) . "</td>";	
						}
					}else{
						if($data['total'] < 0){
							echo "<td align=\"right\" style=\"background-color:".$color."\">" . convert_amount_dc2(abs($data['total'])) . $this->print_space($this->counter) . "</td>";
				...
				if($level=='L3'){
					...
					$CI =& get_instance();
					$CI->load->model('Ledger_model');
					$group = $CI->Ledger_model->get_group($data['parent_id']);
					//$group = $CI->Ledger_model->get_group($group->parent_id);
					echo $group->name;
					echo ' - ';
					echo $data['gname'];
					echo ':';
