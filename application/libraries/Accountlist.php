...
class Accountlist
{
	...
	function add_sub_ac_groups()
	{
		...
		$CI->db->from('ac_groups')->where('parent_id', $this->id)->order_by('group_order','ASC');
		$child_group_q = $CI->db->get();
		$counter = 0;
		foreach ($child_group_q->result() as $row)
		{
			$this->children_ac_groups[$counter] = new Accountlist();
	...
	function add_sub_ac_ledgers()
	{
		...
		$CI->db->from('ac_ledgers')->where('group_id', $this->id);
		$child_ledger_q = $CI->db->get();
		$counter = 0;
		foreach ($child_ledger_q->result() as $row)
		{
			$this->children_ac_ledgers[$counter]['id'] = $row->id;
			$this->children_ac_ledgers[$counter]['active'] = $row->active;
			$this->children_ac_ledgers[$counter]['name'] = $row->name;
			$this->children_ac_ledgers[$counter]['total'] = $CI->Ledger_model->get_ledger_balance($row->id);
			list ($this->children_ac_ledgers[$counter]['opbalance'], $this->children_ac_ledgers[$counter]['optype']) = $CI->Ledger_model->get_op_balance($row->id);
	...
	function account_st_short($c = 0)
	{
		...
		if (count($this->children_ac_ledgers) > 0)
		{
			...
			foreach ($this->children_ac_ledgers as $id => $data)
			{
				...
				echo "<td class=\"td-ledger\">";
				echo $this->print_space($this->counter);
				echo "&nbsp;" . anchor('accounts/report/ac_ledgerst/' . $data['id'], $data['name'], array('title' => $data['name'] . ' Ledger Statement', 'style' => 'color:#000000'));
				echo "</td>";
	...
	function account_st_main($c = 0)
	{
		...
		{
			if ($this->id <= 4){
				echo "<tr class=\"tr-group\" style=\"background:#dff0d8\">";
			}else{
				echo "<tr class=\"tr-group\" style=\"background:#fcf8e3\">";
			}
			echo "<td class=\"td-ledger\">";
			echo "</td>";
			echo "<td class=\"td-group\">";
			...
			echo "<td align=right style=\"padding-right:20px;\">-</td>";
			echo "<td align=right style=\"padding-right:20px;\">-</td>";
			echo "<td align=right style=\"padding-right:20px;\">-</td>";
			echo "<td>-</td>";
...
<!--				</td>-->
		...
		if (count($this->children_ac_ledgers) > 0)
		{
			...
			foreach ($this->children_ac_ledgers as $id => $data)
			{
				//echo "<tr class=\"tr-ledger\">"
				?>
				<tr <? if ($data['active'] == 0){?> class="success" <? }else{ ?> class="<? echo ($c1<0) ? 0 : ($c1++ % 2 == 1) ? 'info' : '';?>" <? }?>>
					<?
				echo "<td class=\"td-ledger\">";
				//Modified code to set there uniformity
				...
				echo "<td align=right style=\"padding-right:20px;\">" . $data['optype'] . "</td>";
				echo "<td align=right style=\"padding-right:20px;\">" . convert_opening($data['opbalance'], $data['optype']) . "</td>";
				echo "<td align=right style=\"padding-right:20px;\">" . convert_amount_dc($data['total']) . "</td>";
				$CI =& get_instance();
				...
				if(ledger_deletable($data['id']) && check_access('delete ledger')){ //custom helper funtion
					echo " &nbsp;" . anchor('accounts/ledger/delete/' . $data['id'], img(array('src' => asset_url() . "images/icons/delete.png", 'border' => '0', 'alt' => 'Delete Ledger')), array('class' => "confirmClick", 'title' => "Delete Ledger", 'onclick' => "return confirm('Do you want delete this record')"));
				}
