...
class Accountlistprd
{
	...
	function add_sub_ac_groups()
	{
		...
		{
			$this->children_ac_groups[$counter] = new Accountlistprd();
	...
	function add_sub_ac_ledgers()
	{
		...
		foreach ($child_ledger_q->result() as $row)
		{
			...
			$balance=$CI->Ledger_model->get_opledger_balance_period($row->id,$this->sttime,$this->endtime);
			$this->children_ac_ledgers[$counter]['opbalance']=$balance;
			if($balance<0)
			$type='Cr';
			else $type='Dr';
			$this->children_ac_ledgers[$counter]['optype']=$type;
