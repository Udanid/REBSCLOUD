...
class Accountlistpnlbs
{
	...
	function add_sub_ac_groups()
	{
		...
		{
			$this->children_ac_groups[$counter] = new Accountlistpnlbs();
	...
	function add_sub_ac_ledgers()
	{
		...
		foreach ($child_ledger_q->result() as $row)
		{
			...
			list ($this->children_ac_ledgers[$counter]['opbalance'], $this->children_ac_ledgers[$counter]['optype']) = $CI->Ledger_model->get_opledger_balance_period($row->id,$this->sttime,$this->endtime);
		...
		if (count($this->children_ac_ledgers) > 0)
		{
			...
			foreach ($this->children_ac_ledgers as $id => $data)
			{
				if($level=='L3'){
					...
					echo "<td align=\"right\">" . convert_amount_dc2($data['total']) . $this->print_space($this->counter) . "</td>";
					echo "</tr>";
				}
				if($level=='L2'){
					echo "<tr class=\"tr-ledger\">";
