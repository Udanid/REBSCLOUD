...
class Accountlistpnl
{
	...
	function add_sub_ac_groups()
	{
		...
		{
			$this->children_ac_groups[$counter] = new Accountlistpnl();
				...
				if($level=='L3'){
					...
					substr($data['id'], 0, 3) .'-'.
					 substr($data['id'], 3, 2) .'-'.
							...
							substr($data['id'], 11, 2);
					//echo $code;
					echo "&nbsp;" . anchor('accounts/report/ac_ledgerst/' . $data['id'], $code, array('title' => $data['name'] . ' Ledger Statement','target' => '_blank'));
					echo "</td>";
					echo "<td class=\"td-ledger\">";
					echo $this->print_space($this->counter);
					echo $data['name'];
