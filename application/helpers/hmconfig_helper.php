...
if ( ! function_exists('check_foreign_key'))
{
	function check_foreign_key($table,$id,$feild_name)
	{
		$CI =& get_instance();
		$CI->load->model('hm_config_model');
		$status = $CI->hm_config_model->check_foreign_key($table,$id,$feild_name);
		return $status;

	}
}

if ( ! function_exists('chkimgexist'))
{
function chkimgexist($table,$id,$feild_name)
    {
        $CI =& get_instance();
        $CI->load->model('hm_config_model');
        $status = $CI->hm_config_model->chkimgexist($table,$id,$feild_name);
        return $status;
