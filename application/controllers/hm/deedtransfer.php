...
class Deedtransfer extends CI_Controller {
	...
	public function index()
	{
		...
		redirect('hm/deedtransfer/showall');
	...
	public function showall()
	{
		...
		if ( ! check_access('view_deedtrn'))
		{
			...
			$data['prjlist']=$this->hm_deedtransfer_model->get_all_project_confirmed($this->session->userdata('branchid'));
			$data['deedlist']=$this->hm_deedtransfer_model->get_deedtranfers_by_type();
				...
				$this->load->view('hm/deedtransfer/deed_search',$data);
	...
	function get_blocklist($id)
{
	...
	$data['lotlist']=$this->hm_deedtransfer_model->get_project_sold_lots($id);
	$this->load->view('hm/deedtransfer/blocklist',$data);
}
	...
	public function get_fulldata()
	{
		...
		$data['lotdetail']=$lotdata=$this->hm_lotdata_model->get_project_lotdata_id($this->uri->segment(4));
		$data['plandata']=$this->hm_deedtransfer_model->get_project_plannumber($this->uri->segment(5),$lotdata->plan_sqid);
		$data['deed_data']=$this->hm_deedtransfer_model->get_deedtranfers_data_byid($this->uri->segment(5),$this->uri->segment(4));
		$data['legal_officer']=$this->hm_deedtransfer_model->get_leagal_officer_list();
		...
		$this->load->view('hm/deedtransfer/deed_data',$data);
		...
		public function get_fulldata_popup()
	{
		...
		$this->load->view('hm/deedtransfer/deed_data_popup',$data);
	...
	function print_lowyercopy()
	{
		...
		$this->load->view('hm/deedtransfer/print_lawyercopy',$data);
	}
	function print_customer()
	{
		...
		$this->load->view('hm/deedtransfer/print_customer',$data);
	}
	public function add_transfer()
	{
		...
		$id=$this->hm_deedtransfer_model->add($file_name);
		
		$this->common_model->add_notification('hm_deedtrn','Deed Transfer','hm/deedtransfer',$id);
		...
		public function update_transfer()
	{
		...
		$id=$this->hm_deedtransfer_model->update_transfer($file_name);
	...
	public function confirm_transfer()
	{
		...
		$id=$this->hm_deedtransfer_model->confirm_transfer($this->uri->segment(4));
		
		$this->common_model->delete_notification('hm_deedtrn',$this->uri->segment(4));
	...
	public function update_deeddata()
	{
		...
		$id=$this->hm_deedtransfer_model->update_deeddata();
	...
	public function upload_cuscopy()
	{
		...
		$file_name="";
				...
				{
            	$file_name = $image_data['file_name'];
				$id=$this->hm_deedtransfer_model->upload_cuscopy($file_name);
	...
	public function confirm_deed()
	{
		...
		$id=$this->hm_deedtransfer_model->confirm_deed($this->uri->segment(4));
