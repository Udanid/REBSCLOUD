...
class Income extends CI_Controller {
    ...
    function Income()
    {
        ...
        $this->load->model('hm_entry_model');
    ...
    function index()
    {
        ...
        $ac_incomes = $this->hm_entry_model->get_pending_incomes($this->session->userdata('branchid'));
		$datalist=NULL;
        if (!$ac_incomes)
        {
            $this->session->set_flashdata('error', 'No Data');
        }
		$data['prjlist']=$this->hm_entry_model->get_all_project_confirmed($this->session->userdata('branchid'));
...
//        $data['projectdata']=$this->ac_projects_model->get_all_ac_projects();
        ...
        // $data=NULL;
        $this->load->view('hm/income/index',$data);
        return;
    }
...
function get_blocklist($id)
{
	...
	$data['lotlist']=$this->hm_entry_model->get_project_notpending_lots_byprjid($id);
	$this->load->view('hm/income/blocklist',$data);
}
function get_fulldata_lotid($id)
{
	
	$data['current_rescode']=$current_rescode=$this->lotdata_model->get_max_resid($id);
	$data['ac_incomes']=NULL;
	...
	if($current_rescode)
	{
		...
		$data['ac_incomes']=$this->hm_entry_model->get_pending_incomes_by_tempcode($current_rescode);
		if($current_res->pay_type!='Outright')
			{
				$data['loan_data']=$loan_data=$this->reservation_model->get_eploan_data($current_rescode);
				if($loan_data){
				$data['ac_incomes2']=$this->hm_entry_model->get_pending_incomes_by_tempcode($loan_data->loan_code);
				}
			}
...
{$data['ac_incomes2']=NULL;
	$data['ac_incomes']=$this->hm_entry_model->get_pending_incomes_by_project($id);
	//echo $id;
	$this->load->view('hm/income/fulldata',$data);
}
function get_fulldata()
{$data['ac_incomes2']=NULL;
	$data['ac_incomes']=$this->hm_entry_model->get_pending_incomes($this->session->userdata('branchid'));
	$this->load->view('hm/income/fulldata',$data);
}
...
//        }
        ...
        $data['income_entry'] = $this->hm_entry_model->get_incomedata_by_entryid($entry_id);
        ...
        $datalist=$this->hm_entry_model->get_incomes_by_entryid($entry_id);
        $cur_entry=$this->hm_entry_model->get_incomesByEntryid($entry_id);
...
//                    $this->template->load('template', 'entrymaster/add', $data);
	...
	hm_update_jurnal_entry_insert($cur_entry->income_type,$cur_entry->id,date('Y-m-d'));
