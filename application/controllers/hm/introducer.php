...
class Introducer extends CI_Controller {
	...
	public function showall()
	{
		...
		{
			$this->session->set_flashdata('error', 'Permission Denied');
			redirect('hm/introducer/');
			return;
		}
		$data['searchdata']=$inventory=$this->hm_introducer_model->get_all_intorducer_summery();
				$courseSelectList="";
				 $count=0;
				...
				$data['searchpath']='hm/introducer/search';
				...
				$page_count = (int)$this->uri->segment(4);
					if ( !$page_count)
					$page_count = 0;
				$data['datalist']=$this->hm_introducer_model->get_all_intorducer_details($pagination_counter,$page_count);
				$siteurl='hm/introducer/showall';
				$tablename='hm_introducerms';
				$this->pagination($page_count,$siteurl,$tablename);
	
				$this->load->view('hm/introducer/introducer_data',$data);
	...
	public function search()
	{
		...
		$data['bankdata']=$this->hm_introducer_model->get_introducer_bankdata_bycode($this->uri->segment(4));
		$data['banklist']=$this->common_model->getbanklist();
		$data['details']=$this->hm_introducer_model->get_intorducer_bycode($this->uri->segment(4));
		$this->load->view('hm/introducer/search',$data);
	...
	public function add()
	{
		...
		$id=$this->hm_introducer_model->add($file_name);
		
		$this->common_model->add_notification('hm_introducerms','Introducers','hm/introducer',$id);
	...
	public function edit()
	{
		...
		$data['bankdata']=$this->hm_introducer_model->get_introducer_bankdata_bycode($this->uri->segment(4));
		$data['banklist']=$this->common_model->getbanklist();
		$this->common_model->add_activeflag('hm_introducerms',$this->uri->segment(4),'intro_code');
		$session = array('activtable'=>'hm_introducerms');
		$this->session->set_userdata($session);
		$this->load->view('hm/introducer/edit',$data);
	...
	function editdata()
	{
		...
		$id=$this->hm_introducer_model->edit($file_name);
		
		
		$this->session->set_flashdata('msg', 'Introducer Details Successfully Updated ');
		
		$this->common_model->delete_activflag('hm_introducerms',$this->input->post('intro_code'),'intro_code');
	...
	public function confirm()
	{
		...
		$id=$this->hm_introducer_model->confirm($this->uri->segment(4));
		
		$this->common_model->delete_notification('hm_introducerms',$this->uri->segment(4));
		...
		public function delete()
	{
		...
		$id=$this->hm_introducer_model->delete($this->uri->segment(4));
