...
class Documents extends CI_Controller {
	...
	public function showall()
	{
		...
		$data['searchdata']=$inventory=$this->customer_model->get_all_customer_summery();
				...
				foreach($inventory as $c)
				 ...
				 $data['prjlist']=$this->hm_document_model->get_project_list();
				 $data['landlist']=$this->hm_document_model->get_land_list();
				$data['searchlist']=$courseSelectList;
				$data['searchpath']='hm/customer/search';
				
				$this->load->view('hm/documents/document_data',$data);
	...
	public function land_docs()
	{
		...
		{
			$this->session->set_flashdata('error', 'Permission Denied');
			//redirect('hm/documents/');
			return;
		}
		$data['details']=$this->hm_document_model->get_land_documents($this->uri->segment(4));
		$this->load->view('hm/documents/land_docs',$data);
   ...
   public function update_landdoc()
   {
	   if ( ! check_access('edit_customer'))
		...
		$doclist=$this->hm_document_model->get_land_documents($landcode);
		$file_name="";
		...
		foreach($doclist as $raw)
		{
			...
			$this->hm_document_model->update_land_documents($id,$plan_no,$deed_no,$drawn_by,$drawn_date,$attest_by,$attest_date,$plan_copy,$deed_copy);
	...
	public function project_docs()
	{
		...
		$data['typelist']=$doctypes=$this->hm_document_model->get_document_bycategory('Projects');
		$doclist=NULL;
		foreach($doctypes as $raw)
		{
			$prjdoc=$this->hm_document_model->get_project_documents($prj_id,$raw->doctype_id);
			if($prjdoc)
		...
		$this->load->view('hm/documents/project_docs',$data);
		...
		$doctypes=$this->hm_document_model->get_document_bycategory('Projects');	
			...
			{ 
				...
				if($error==""){
					...
					$this->hm_document_model->delete_project_documents($id,$docraw->doctype_id,$filename);
					$this->hm_document_model->add_project_documents($id,$docraw->doctype_id,$filename);
				}
				else
