...
class Commission extends CI_Controller {
	 ...
	 */
	 function __construct() {
		...
		$this->load->model("hm_commission_model");
	...
	if ( ! check_access('view commissionrates'))
		...
		$data['searchdata']=$inventory=$this->hm_commission_model->get_commission_cat();
		$data['usertype']=$usertype=$this->hm_commission_model->get_commission_cat();
		
	$this->load->view('hm/commission/config_main',$data);
	...
	public function add_range()
	{
		...
		$id=$this->hm_commission_model->add_range();
	...
	public function delete_range()
	{
		...
		$id=$this->hm_commission_model->delete_range($this->uri->segment(4));
	...
	function get_categodtylist()
	{
		...
		$data['catlist']=$this->hm_commission_model->get_commission_cat_year($year);
		$this->load->view('hm/commission/config_catlist',$data);
	}
	...
	public function add_rate()
	{
		...
		$data['catlist']=$catlist=$this->hm_commission_model->get_commission_cat_year($year);
		if($catlist)
		{
			foreach($catlist as $raw)
			{
				 ...
				 $amount=str_replace(',', '', $this->input->post('amount'.$raw->id));
				  ...
				  $status='PENDING';
							  ...
							  if($status=='PENDING')
							  {
							  $this->hm_commission_model->delete_cat_rate($raw->id,$tableid);
							  $this->hm_commission_model->insert_cat_rate($amount,$raw->id,$tableid,$year,$rate_type);
							  
							  }
							  
			}
			$this->hm_commission_model->update_cat_status($year);
		}
	...
	if ( ! check_access('view Commision list'))
		...
		$data['searchdata']=$inventory=$this->hm_commission_model->get_commission_master();
		
	$this->load->view('hm/commission/commission_main',$data);
	...
	function generate_commission()
	{
		if ( ! check_access('generate_commision'))
        {
			...
			$checkgenerated=$this->hm_commission_model->get_checkgenerated($year,$month);
			if($checkgenerated)
			{	
			
				 $startdate=$year.'-'.$month.'-01';
				 $enddate=$year.'-'.$month.'-31';
				$currentmonthpayment=$this->hm_commission_model->get_current_month_payment($startdate,$enddate);
				$runid=$this->hm_commission_model->add_commision_master($year,$month);
				if($currentmonthpayment)
				{
					foreach($currentmonthpayment as $raw)
					{
						...
						$resdata=$this->hm_commission_model->get_all_reservation_details_bycode($res_code);
						
							$price=$resdata->discounted_price;
							$downpayment=$downpayment=$this->hm_commission_model->tot_month_payment($res_code,$enddate);
							...
							if($presentage>=40)
							{
										...
										$is_commigiv=$this->hm_commission_model->is_commission_given($resdata->lot_id);
										if($is_commigiv)
										{
					...
					redirect("hm/commission/commission_main");
			}
			else
			{
...
// @link http://www.php.net/manual/en/class.dateinterval.php
			$diff=date_diff($d1,$d2);
			 $months= $diff->format('%m');
			  $days= $diff->format('%a');
			$presentage=($downpayment/$price)*100;
			//$category=$this->hm_commission_model->get_price_category($year,$price);
			$commision=0;
		//	echo $category.'ssss'.number_format($price,2);
			//$commision=$this->hm_commission_model->get_commission_rate_focal($category,$table);
		$tableid=0;
		$dataid=0;
		if($futureDate >= $pastpaydate)
		{
			 if($days<=21)
			 {
				 ...
				 {
					  $tableid=1;
					  $commision=$this->hm_commission_model->get_commission_rate_focal($runid,$res_code,$lot_id,$prj_id,$year,1,$price,$team_leader,$tpo,$officer_code,$officer_code2,$pastpaydate,$presentage);
		 ...
		 else
		 {
			 ...
			 {
				  $tableid=3;
				  $commision=$this->hm_commission_model->get_commission_rate_focal($runid,$res_code,$lot_id,$prj_id,$year,3,$price,$team_leader,$tpo,$officer_code,$officer_code2,$pastpaydate,$presentage);
	...
	{
		$dataset=$this->hm_commission_model->commission_staff_data($prevrun_id,$previd);
		if($dataset)
		{
			foreach($dataset as $raw)
			{
				...
				$amount=0;
				$this->hm_commission_model->add_commision_staff_data($runid,$dataid,$emp_id,$amount,$deduct_amount,$deduct_runid,$deduct_data_id);
	...
	public function delete_commission()
	{
		...
		$id=$this->hm_commission_model->delete_commission($this->uri->segment(4));
	...
	function get_commisiondata()
	{
		...
		$staffdata=NULL;
		$tpodata=NULL;
		$data['rundata']=$rundata=$this->hm_commission_model->get_commission_year_month($year,$month);
		if($rundata){
		$data['commisiondata']=$commisiondata=$this->hm_commission_model->commission_lot_run_id($rundata->id);
		$data['staffsummery']=$staffsummery=$this->hm_commission_model->get_current_month_staffsummery($rundata->id);
			$data['projectsummery']=$projectsummery=$this->hm_commission_model->get_current_month_projectsummery($rundata->id);
			
		//	$data['commisiondata']=$commisiondata=$this->hm_commission_model->commission_lot_run_id_prjid($runid,$projectid); a
		if($staffsummery)
		...
		if($projectsummery)
		{
			...
			//	
				$staffdata[$staffraw->emp_id][$row->project_id]=$this->hm_commission_model->commission_staff_prjid_sum($rundata->id,$row->project_id,$staffraw->emp_id);
				
				$tpodata[$row->project_id]=$this->hm_commission_model->commission_tpo_prjid_sum($rundata->id,$row->project_id);
		...
		$this->load->view('hm/commission/commission_data',$data);
		}
	}
	function get_commision_staff()
	{
		...
		$data['staffdata']=NULL;
		$tpodata=NULL;
		$data['rundata']=$rundata=$this->hm_commission_model->get_commission_runid($runid);
		$data['commisiondata']=$commisiondata=$this->hm_commission_model->commission_lot_run_id_prjid($runid,$projectid);
		if($commisiondata)
		{
			foreach($commisiondata as $row)
			{
				
				$staffdata[$row->id]=$this->hm_commission_model->commission_staff_prjid($row->id);
				$tpodata[$row->id]=$this->hm_commission_model->commission_tpo_prjid($row->id);
				
			}
		}
		$data['prjdata']=$prjdata=$this->hm_commission_model->get_project_master_data($projectid);
		$data['allstaff']=$allstaff=$this->hm_commission_model->get_all_sales_staff($prjdata->branch_code);
		$data['staffdata']=$staffdata;
		$data['tpodata']=$tpodata;
		$this->load->view('hm/commission/commission_staffdata',$data);
	}
	function staff_commission_update()
	{
		...
		$data['staffdata']=NULL;
		$data['prjdata']=$prjdata=$this->hm_commission_model->get_project_master_data($projectid);
		$data['commisiondata']=$commisiondata=$this->hm_commission_model->commission_lot_update_staffamount($runid,$projectid,$prjdata->branch_code);
	...
	public function confirm_commission()
	{
		...
		$id=$this->hm_commission_model->confirm_commission($this->uri->segment(4));
	...
	function commission_details_excel()
	{
		...
		$data['staffdata']=$staffdata;
		$data['tpodata']=$tpodata;
		$this->load->view('hm/commission/commission_data_excel',$data);
		}
	}
		function commission_details_staff_excel()
	{
		...
		$data['staffdata']=$staffdata;
		$data['tpodata']=$tpodata;
		$this->load->view('hm/commission/commission_data_staff_excel',$data);
		}
	}
