...
class Customerletter extends CI_Controller {
	...
	public function index()
	{
		...
		redirect('hm/customerletter/showall');
	...
	public function showall()
	{
		...
		$data['searchdata']=$inventory=$this->hm_message_model->get_letter_types();
		...
		$data['prjlist']=$this->hm_project_model->get_all_project_summery($this->session->userdata('branchid'));
				...
				$this->load->view('hm/customerletter/leter_data',$data);
	...
	public function letter_list()
	{
		...
		{
			$this->session->set_flashdata('error', 'Permission Denied');
			redirect('hm/customerletter/');
			return;
		}
		$data['type_id']=$this->uri->segment(4);
		$data['type_name']=get_letter_type($this->uri->segment(4));
	//	if($this->uri->segment(4)=='04')
		//$thislist=$this->hm_message_model->generate_first_remind();
	//	if($this->uri->segment(4)=='05')
	//	$thislist=$this->hm_message_model->generate_second_remind();
	//	if($this->uri->segment(4)=='06')
	//	$thislist=$this->hm_message_model->generate_third_remind();
	//	if($this->uri->segment(4)=='07')
		//$thislist=$this->hm_message_model->generate_termination_letter();
	//	if($this->uri->segment(4)=='08')
	//	$thislist=$this->hm_message_model->generate_resale_letter();
		$data['datalist']=$this->hm_message_model->get_letters_by_type($this->uri->segment(4),$this->session->userdata('branchid'));
		$this->load->view('hm/customerletter/letter_list',$data);
	...
	public function printed_letter_list()
	{
		...
		$data['type_id']=$this->uri->segment(4);
		$data['type_name']=get_letter_type($this->uri->segment(4));
		$data['datalist']=$this->hm_message_model->get_letters_printed_lettets($this->uri->segment(4),$this->uri->segment(5));
		$this->load->view('hm/customerletter/printed_letter_list',$data);
	...
	public function letter_print()
	{
		...
		$data['letter_data']=$letter_data=$this->hm_message_model->get_letter_byid($this->uri->segment(4));
		$data['customer_data']=$this->hm_message_model->get_customer_bycode($letter_data->cus_code);
		$data['res_data']=$this->hm_reservation_model->get_all_reservation_details_bycode($letter_data->res_code);
		if($letter_data->loan_code)
		{
		 $data['details']=$loan_data=$this->hm_eploan_model->get_eploan_data($letter_data->loan_code);
		  $data['rshceduls']=false;
		  $data['rebate']=false;
			  $data['currentins']=$this->hm_eploan_model->get_current_instalment($letter_data->loan_code);
		 	}
		
		$this->load->view('hm/customerletter/customerletter_'.$this->uri->segment(5),$data);
	...
	public function update_printstatus($id)
	{
		...
		$data['datalist']=$this->hm_message_model->update_print_status($this->uri->segment(4));
		redirect('hm/customerletter/');
	...
	public function add()
	{
		...
		$this->common_model->add_notification('cm_customerms','customers','hm/customer',$id);
