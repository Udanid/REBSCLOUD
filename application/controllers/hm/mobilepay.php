...
class mobilepay extends CI_Controller {
    ...
    function mobilepay()
    {
        ...
        $this->load->model('hm_budgeting_model');
		 ...
		 $this->load->model('hm_common_model');
		  ...
		  $this->load->model('hm_reservation_model');
		   $this->load->model('hm_lotdata_model');
		    $this->load->model('hm_mobilepay_model');
	$this->is_logged_in();
    ...
    function index()
    {
        ...
        $ac_incomes = $this->hm_mobilepay_model->get_pending_incomes();
		$datalist=NULL;
        if (!$ac_incomes)
        {
        ...
        $this->load->view('hm/mobilepay/index',$data);
        return;
    }
...
function advancepay($tid)
{
  // echo $tid;
  // break;
	$paymentdata=$this->hm_mobilepay_model->get_mobilepaydata_id($tid);
	if($paymentdata){
	$id=$this->hm_mobilepay_model->add_advance($paymentdata->amt,$paymentdata->loanId,$paymentdata->bill_num,$paymentdata->timestmp);

		$this->hm_mobilepay_model->update_mobile_pay($tid,$id);
		...
		redirect("hm/income/add/".$id);
	}
	else
	{
		$this->session->set_flashdata('error', 'Invalid paymentttt');
		redirect("hm/mobilepay/");
	}
...
function rentalpay($tid)
{
	...
	$paymentdata=$this->hm_mobilepay_model->get_mobilepaydata_id($tid);
	if($paymentdata){
		$this->hm_mobilepay_model->update_mobile_pay($tid,'');
			$data['mobdetails']= $this->hm_mobilepay_model->get_project_payment_data_by_code($paymentdata->loanId);
		$data['loan_code']=$loancode=$paymentdata->loanId;
		$mydate=date('Y-m-d',$paymentdata->timestmp);
		 $data['temp_rct_no']=$temp_rct_no=$paymentdata->bill_num;
    $data['temp_rct_date']=$temp_rct_date=$mydate;
		...
		$this->load->view('hm/mobilepay/rentaldata',$data);
		else
		{
			...
			$this->load->view('hm/mobilepay/rentaldata_epb',$data);
		}
	}
	else
	{
...
{$data['ac_incomes2']=NULL;
	...
	//echo $id;
	$this->load->view('hm/income/fulldata',$data);
}
...
//                $this->form_validation->set_rules('cr_amount[' . $id . ']', 'Credit Amount', 'trim|currency');
        ...
        if ( ! $_POST)
        {
            ...
            $this->db->from('hm_incomentires')->where('income_id', $entry_id)->order_by('dc_type','desc');
...
//            $this->load->view('hm/payments/edit',$data);
//            return;
//        }
//        else
//        {
        ...
        if($data_all_ledger_dc) {
            ...
            foreach ($data_all_ledger_dc as $id => $ledger_data) {
                ...
                if ($valid_ledger_q->num_rows() < 1) {
                    ...
                    //$this->template->load('template', 'payments/edit', $data);
                    $this->load->view('hm/income/add', $data);
                    return;
                } else {
...
//                        $non_bank_cash_present = TRUE;
        ...
        if(! $this->db->where('id',$entry_id)->update('hm_prjacincome',$update_data))
        {
            $this->db->trans_rollback();
            $this->session->set_flashdata('error', 'Error updating hm_prjacincome.');
            $this->load->view('hm/income/add',$data);
            return;
        }
...
//                $this->template->load('template', 'entrymaster/add', $data);
        ...
        if($this->reciept_model->is_last_reciept($reciptid))
        {
            ...
            if ( !$this->db->where('RCTBID',$bookid)->update('recieptbookdata', $rcdnewdata))
            {
                ...
                //$this->logger->write_message("error", "Error Inserting Reciept Data");
//                    $this->template->load('template', 'entrymaster/add', $data);
                $this->load->view('hm/entrymaster/add',$data);
	...
	update_jurnal_entry_insert($cur_entry->income_type,$cur_entry->id,date('Y-m-d'));
     ...
     //   $this->logger->write_message("success", "Added Income Entry number " . full_entry_number($entry_type_id, $data_number) . " [id:" . $entry_id . "]");
      ...
      //  redirect('hm/entrymaster/show/receipt');
		redirect('hm/entrymaster/printreciepts/' .$new_entry_id);
...
//        if ($entry_type == 'tag')
//        {
//            $config['base_url'] = site_url('hm/entrymaster/show/tag' . $tag_id);
//            $config['uri_segment'] = 5;
//        } else if ($entry_type == 'all') {
//            $config['base_url'] = site_url('hm/entrymaster/show/all');
//            $config['uri_segment'] = 4;
//        } else {
//            $config['base_url'] = site_url('hm/entrymaster/show/' . $current_entry_type['label']);
//            $config['uri_segment'] = 4;
//        }
        ...
        if($amountsearch!="" && $voucher_no != "" ){
            ...
            $this->db->from('hm_prjacincome')->where('id', $voucher_no)->where('amount',$amountsearch);
            $entry_q = $this->db->get();
            $rowcount = $this->db->from('hm_prjacincome')->where('id', $voucher_no)->where('amount',$amountsearch)->get()->num_rows();
            if($rowcount<1)
            {
        ...
        {
            //$amountsearch=number_format($amountsearch, 2, '.', '');
            $this->db->from('hm_prjacincome')->where('amount', $amountsearch)->where('pay_status','PENDING')->order_by('income_date', 'desc');
            //$this->db->join('tchdata','tchdata.TCHID=accclbook.TCHID');
            $entry_q = $this->db->get();
            $rowcount = $this->db->from('hm_prjacincome')->where('amount', $amountsearch)->where('pay_status','PENDING')->order_by('income_date', 'desc')->get()->num_rows();
            if($rowcount<1)
            {
                //$this->messages->add('Receipt number not exist ', 'error');
        ...
        else if($amountsearch=="" && $voucher_no!="" )
        {
            ...
            $this->db->from('hm_prjacincome')->where('id', $voucher_no);
            //$this->db->join('tchdata','tchdata.TCHID=accclbook.TCHID');
            $entry_q = $this->db->get();
            $rowcount = 	$this->db->from('hm_prjacincome')->where('id', $voucher_no)->get()->num_rows();
            if($rowcount<1)
            {
                //$this->messages->add('Receipt number not exist ', 'error');
                $this->session->set_flashdata('error', 'Voucher number not exist');
        ...
        $this->load->view('hm/income/search',$data);
        return;
 ...
 function get_reservations()
    {
        ...
        /* Calculating difference in Opening Balance */
        //$ac_projects = $this->paymentvoucher_model->get_pending_payment_vouchers($this->session->userdata('branchid'));
       $data['searchdata']= $ac_incomes = $this->hm_mobilepay_model->get_pending_reservations();
			$data['searchlist']='';
				$data['searchpath']='';
        ...
        $this->load->view('hm/mobilepay/reservation_list',$data);
        return;
    }
 function confirm_res($id)
 {

		$this->hm_mobilepay_model->confirm_res($id);
		$mobdata=$this->hm_mobilepay_model->confirm_data_byid($id);
