...
class Land extends CI_Controller {
	...
	public function showall()
	{
		...
		$data['searchdata']=$inventory=$this->hm_land_model->get_all_land_summery();
				$courseSelectList="";
				 $count=0;
				...
				$data['searchpath']='hm/land/search';
				...
				$data['introduceerlist']=$this->hm_introducer_model->get_all_intorducer_summery();
		...
		/* Pagination configuration */
			...
			$config['base_url'] = site_url('hm/land/showall');
		...
		$data['datalist']=$this->hm_land_model->get_all_land_details($pagination_counter,$page_count);
		$data['landnames']=$this->hm_land_model->get_all_land_names();
		$config['total_rows'] = $this->db->count_all('hm_landms');
	...
	$this->load->view('hm/land/land_data',$data);
	...
	public function search()
	{
		...
		$data['details']=$this->hm_land_model->get_land_bycode($this->uri->segment(4));
		$data['product_code']='LNS';
				$data['district_list']=$this->common_model->distict_list();
				$data['council_list']=$this->common_model->council_list();
				$data['town_list']=$this->common_model->town_list();
				$data['introduceerlist']=$this->hm_introducer_model->get_all_intorducer_summery();
		$this->load->view('hm/land/search',$data);
	...
	public function add()
	{
		if ( ! check_access('add_land'))
		{
			...
			return;
				...
				$this->session->set_flashdata('error',  $error );
		$id=$this->hm_land_model->add($plan_copy,$deed_copy);
	//	print_r($_POST);
		//print_r($_FILES);
		$this->hm_land_model->add_owners($id, $this->input->post('owner_name'),$this->input->post('owner_address'),$this->input->post('owner_nic'));
		$this->hm_land_model->add_documents($id,$this->input->post('plan_no'),$this->input->post('deed_no'),$this->input->post('drawn_by'),$this->input->post('drawn_date'),$this->input->post('attest_by'),$this->input->post('attest_date'),$plan_copy,$deed_copy);
		for($i=1; $i<=4; $i++)
		{
			...
			if($name!="")
				$this->hm_land_model->add_owners($id,$name,$address,$nic);
			...
			//echo $error;
				...
				if($error=="")
            	$deed_copy =$image_data['file_name'];
				$this->hm_land_model->add_documents($id,$plan_no,$deed_no,$drawn_by,$drawn_date,$attest_by,$attest_date,$plan_copy,$deed_copy );
		...
		$this->common_model->add_notification('hm_landms','lands','hm/land',$id);
	...
	public function edit()
	{
		...
		$data['details']=$this->hm_land_model->get_land_bycode($this->uri->segment(4));
		$data['owners']=$this->hm_land_model->get_land_owners($this->uri->segment(4));
		$data['docs']=$this->hm_land_model->get_land_documents($this->uri->segment(4));
		$data['product_code']='LNS';
				$data['district_list']=$this->common_model->distict_list();
				$data['council_list']=$this->common_model->council_list();
				$data['town_list']=$this->common_model->town_list();
				$data['introduceerlist']=$this->hm_introducer_model->get_all_intorducer_summery();
		$this->common_model->add_activeflag('hm_landms',$this->uri->segment(4),'land_code');
		$data['landnames']=$this->hm_land_model->get_all_land_names();
		$session = array('activtable'=>'hm_landms');
		$this->session->set_userdata($session);
		$this->load->view('hm/land/edit',$data);
	...
	function comments()
	{
		...
		$data['details']=$this->hm_land_model->get_land_bycode($this->uri->segment(4));
		$data['comments']=$this->hm_land_model->get_land_comments($this->uri->segment(4));

		$this->load->view('hm/land/comments',$data);
	...
	function commetnadd()
	{
		...
		$id=$this->hm_land_model->commetnadd();
		redirect('hm/land/showall');
	}
	function editdata()
	{
		...
		$file_name="";
			...
			$id=$this->hm_land_model->edit($plan_copy,$deed_copy);
			$id=$this->input->post('land_code');
			$this->hm_land_model->delete_owners($id);
			for($i=1; $i<=10; $i++)
			{
		...
		$this->common_model->delete_activflag('hm_landms',$this->input->post('land_code'),'land_code');
	...
	public function confirm()
	{
		...
		$id=$this->hm_land_model->confirm($this->uri->segment(4));

		$this->common_model->delete_notification('hm_landms',$this->uri->segment(4));
		...
		public function delete()
	{
		...
		$id=$this->hm_land_model->delete($this->uri->segment(4));
	...
	public function view()
{
	...
	$session = array('activtable'=>'hm_landms');
	$this->session->set_userdata($session);
	$this->load->view('hm/land/view',$data);
