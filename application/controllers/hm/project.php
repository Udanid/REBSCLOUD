...
class Project extends CI_Controller {
	 ...
	 */
	 function __construct() {
		...
		$this->load->model("hm_document_model");
	...
	public function showall()
	{
		...
		$data['datalist']=$this->hm_project_model->get_all_project_details($pagination_counter,$page_count,$this->session->userdata('branchid'));
	...
	$this->load->view('hm/project/project_data',$data);
	...
	function landinformation()
	{
		...
		$data['doctypes']=$this->hm_document_model->get_document_bycategory('Projects');
		...
		$this->load->view('hm/project/landinformation',$data);
	...
	public function search()
	{
		...
		$data['product_code']='LNS';
				...
				$data['introduceerlist']=$this->hm_introducer_model->get_all_intorducer_summery();
		$this->load->view('hm/project/search',$data);
...
$id=$this->hm_project_model->add();
	//$id=2;
		$file_name="";
			...
			$doctypes=$this->hm_document_model->get_document_bycategory('Projects');
			...
			foreach($doctypes as $docraw)
			{
				...
				if($error==""){
					...
					$this->hm_project_model->add_documents($id,$docraw->doctype_id,$filename);
				}
				else
		...
		$tasklist=$this->hm_producttasks_model->get_tasks_product_code('LNS');
			...
			{
				$this->hm_feasibility_model->update_prjpayment($id,$taskraw->task_id,0.00);
			}
			//$create_boqs=$this->add_unitboq($id);
	...
	public function edit()
	{
		...
		$data['product_code']='LNS';
				...
				$data['council_list']=$this->common_model->council_list();
				$data['introduceerlist']=$this->hm_introducer_model->get_all_intorducer_summery();
		$this->common_model->add_activeflag('hm_projectms',$this->uri->segment(4),'prj_id');
		$session = array('activtable'=>'hm_projectms');
		$this->session->set_userdata($session);
		$this->load->view('hm/project/edit',$data);
	...
	function comments()
	{
		...
		$data['details']=$this->hm_project_model->get_project_bycode($this->uri->segment(4));
		$data['comments']=$this->hm_project_model->get_project_comments($this->uri->segment(4));

		$this->load->view('hm/project/comments',$data);
	...
	function commetnadd()
	{
		...
		$id=$this->hm_project_model->commetnadd();
		redirect('hm/project/showall');
	}
	function editdata()
	{
		...
		$file_name="";
			...
			$id=$this->hm_project_model->edit($plan_copy,$deed_copy);

			//$create_boqs=$this->add_unitboq($this->input->post('prj_id'));
		$this->session->set_flashdata('msg', 'project Details Successfully Updated ');

		$this->common_model->delete_activflag('hm_projectms',$this->input->post('prj_id'),'prj_id');
	...
	public function confirm()
	{
		...
		$id=$this->hm_project_model->confirm($this->uri->segment(4));
