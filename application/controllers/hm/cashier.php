...
class Cashier extends CI_Controller {
	 ...
	 */
	 function __construct() {
		...
		$this->load->model("hm_banktrn_model");
	...
	if ( ! check_access('add_loanpayment'))
		{
			...
			$this->load->view('hm/cashier/monthly_rental',$data);
}
function get_rentalpaydata()
{
	...
	$this->load->view('hm/cashier/rentaldata',$data);
	else
	{
		$data['paylist']=$this->eploan_model->get_paid_list_epb($this->uri->segment(4));
		$this->load->view('hm/cashier/rentaldata_epb',$data);
	}
...
public function showall_reservation()
	{
		...
		$data['searchdata']=$inventory=$this->hm_reservation_model->get_all_reservation_summery($this->session->userdata('branchid'));
				...
				$page_count = (int)$this->uri->segment(4);
					if ( !$page_count)
					$page_count = 0;
				$data['datalist']=$this->hm_reservation_model->get_all_reservation_details($pagination_counter,$page_count,$this->session->userdata('branchid'));
				...
				$this->load->view('hm/cashier/reservation_list',$data);
	...
	{			$res_code=$this->uri->segment(4);
				...
				$this->load->view('hm/cashier/advance_data',$data);
	}
	function add_advance()
	{
		...
		{
			$this->session->set_flashdata('error', 'Permission Denied');
			redirect('hm/cashier/showall_reservation');
	...
	public function reservation_charges()
	{
		...
		$data['searchdata']=$inventory=$this->hm_reservation_model->get_charge_list($this->session->userdata('branchid'));
				$courseSelectList="";
				 $count=0;
				 if($inventory){
				foreach($inventory as $c)
				...
				$status='COMPLETE';
				$this->pagination_status($page_count,$siteurl,$tablename,$statusfield,$status);
				$this->load->view('hm/cashier/reservation_charges',$data);
	...
	{			$res_code=$this->uri->segment(5);
				...
				$this->load->view('hm/cashier/charge_data',$data);
	}
	function add_charges()
	{
		...
		$id=$this->hm_reservation_model->add_charge_payments();
	...
	function delete_charge()
	{
		...
		$id=$this->hm_reservation_model->delete_charge_payment($this->uri->segment(4));
				$this->session->set_flashdata('msg', 'advance payment Successfully Deleted ');
		$this->logger->write_message("success", $this->uri->segment(4).' Advance payment  successfully Deleted');
		redirect("hm/cashier/reservation_charges");
	}
	function banktransfers()
	{
		...
		$data['searchlist']=NULL;
				...
				$data['searchpath']='re/eploan/search';
				 $data['ac_incomes']=$this->hm_banktrn_model->get_bank_trnlist($this->session->userdata('branchid'));
				$data['searchlist']=$courseSelectList;
				$data['banks']=$this->ledger_model->get_all_ac_ledgers_tomake_payment();
				
				
			$this->load->view('hm/banktrn/banktrn_main',$data);
	}
	...
	if ( ! check_access('add_banktrn'))
		{
		 ...
		 $data['ac_incomes']=$this->hm_banktrn_model->search_bank_trnlist($this->uri->segment(4));
		 $this->load->view('hm/banktrn/banktrn_search',$data);
	}
	...
	if ( ! check_access('add_banktrn'))
		...
		$data['ac_incomes']='';
		$data['ac_incomes']=$this->hm_banktrn_model->get_pending_trnlist($this->uri->segment(4));
		 //$this->load->view('accounts/banktrn/banktrn_search',$data);
	//	 
		 $this->load->view('hm/banktrn/search_branch_income',$data);
	}
	function transfer_list()
	{
		...
		for($i=1; $i<=$rowmatcount; $i++)//get selected voucher list
        {
            ...
            {	//$counter++;
				...
				$this->hm_banktrn_model->add_transfer($branch_code,$pay_id,$bankname,$trn_amount,$entry,$trndate);
				$this->hm_banktrn_model->update_incometable($pay_id,$newtransfer,$newbalance,$status);
               
            }
        }
				$this->session->set_flashdata('msg', 'Cash Successfully Transferd');
		$this->logger->write_message("success", ' Cash Successfully Transferd');
		redirect("hm/cashier/banktransfers");
	}
	function search_entry()
	{
		...
		$data['entry_id']=$this->uri->segment(4);
		 $data['ac_incomes']=$this->hm_banktrn_model->search_entry($this->uri->segment(4));
		 $this->load->view('hm/banktrn/search_entry',$data);
	}
	function print_entry()
	{
		...
		$data['entry_id']=$this->uri->segment(4);
		 $data['ac_incomes']=$this->hm_banktrn_model->search_entry($this->uri->segment(4));
		 $this->load->view('hm/banktrn/print_entry',$data);
	}
	function delete_entry()
	{
		...
		$data['entry_id']=$this->uri->segment(4);
		 $data['ac_incomes']=$this->hm_banktrn_model->delete_transfer($this->uri->segment(4));
		 redirect("hm/cashier/banktransfers");
	}
}
