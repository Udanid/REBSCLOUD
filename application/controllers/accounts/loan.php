...
class Loan extends CI_Controller {
  ...
  function editloan_later()
  {
    ...
    $viewData['banknames']=$banknames;
    $viewData['lots_no']=$lots_no;
    $this->load->view('accounts/loan/editloan_afterapprove',$viewData);
  }
  ...
  function payment()
  {
    ...
    $data['cap_loan_numbers']=$cap_loan_numbers=$this->loan_model->get_loans_numbers();
  ...
  function get_report()
  {
    ...
    if($datalist){
      foreach ($datalist as $key => $value) {
        $paid_details[$value->id]=$this->loan_model->getpaid_details_byid($value->id);
        $banknames[$value->bank_code]=$this->loan_model->getbank_byid($value->bank_code);
        $assetName[$value->asset_id]=$this->loan_model->getasset_byid($value->asset_id);
        $prjName[$value->asset_id]=$this->loan_model->getprj_byid($value->asset_id);
        if($value->loan_type=='mortgage'){
          //$lots_no[$loan_byid->id]=$this->loan_model->getlot_no_byid($loan_byid->id);
          $blockList[$value->id]=$this->loan_model->get_prj_lotlist($value->asset_id,$value->id);
        }
      }
    }
    ...
    if($code=='01'){
      ...
      $this->load->view('accounts/loan/report/externalLoanMorgageReport',$data);
    }
  ...
  function report_excel()
  {
    ...
    if($code=='01'){
      ...
      $this->load->view('accounts/loan/report/externalLoanMorgageReport_excel',$data);
    }
  ...
  /*mortgage release start*/
  function mortgage_release()
  {
    if ( ! check_access('outside_loan'))
    {
      $this->session->set_flashdata('error', 'Permission Denied');
    ...
    $lots_no=Null;
    $prj=Null;
    $data['released_mortgage']=$released_mortgage=$this->loan_model->get_mortage_relese_data_loans();
    if($released_mortgage){
      foreach ($released_mortgage as $key => $value) {
        $lots_no[$value->id]=$this->loan_model->get_prj_lotlist($value->asset_id,$value->id);
        $prj[$value->id]=$this->loan_model->getprj_byid($value->asset_id);
      }
    }
    $data['prj']=$prj;
    $data['lots_no']=$lots_no;
    $data['datalist'] =$datalist= $this->loan_model->get_loans();
    $this->load->view('accounts/loan/mortgage_release',$data);
  }
  function mortgage_release_data()
  {
    //$data['id']=$id=$this->uri->segment(4);
    $viewData['loan_byid'] =$loan_byid= $this->loan_model->get_loan_byid($this->uri->segment(4));
    $viewData['prj']=$prj=$this->loan_model->getprj_byid($loan_byid->asset_id);

    $viewData['lots_no']=$lots_no=$this->loan_model->get_prj_lotlist($loan_byid->asset_id,$loan_byid->id);
    $this->load->view('accounts/loan/mortgage_release_data',$viewData);
  }
  function mortgage_update()
  {
    $id=$this->loan_model->mortgage_update($id);

    $this->session->set_flashdata('msg', 'Loan Successfully updated ');
    $this->logger->write_message("success", $this->input->post('loan_id').' Loan successfully Inserted');
    redirect("accounts/loan/mortgage_release");
  }
  /*mortgage release end*/
