...
function advancepay($tid)
{
	...
	if($paymentdata){
	$id=$this->mobilepay_model->add_advance($paymentdata->amt,$paymentdata->loanId,$paymentdata->bill_num,$paymentdata->timestmp);
...
function rentalpay($tid)
{
	...
	$paymentdata=$this->mobilepay_model->get_mobilepaydata_id($tid);
  $loancode=$this->mobilepay_model->get_mobilepaydata_loan_code($paymentdata->loanId);
	if($paymentdata){
...
//        }
 ...
 function confirm_res($id)
 {
		...
		$this->session->set_flashdata('mobile', $mobdata->mobile);
