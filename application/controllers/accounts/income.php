...
class Income extends CI_Controller {
    ...
    function index()
    {
        ...
        $ac_incomes = $this->Entry_model->get_pending_incomes($this->session->userdata('branchid'));
		$datalist=NULL;
        if (!$ac_incomes)
        {
...
//        $data['projectdata']=$this->ac_projects_model->get_all_ac_projects();
        ...
        // $data=NULL;
        $this->load->view('accounts/income/index',$data);
        return;
    }
...
function get_blocklist($id)
{
	...
	$data['lotlist']=$this->Entry_model->get_project_notpending_lots_byprjid($id);
	$this->load->view('accounts/income/blocklist',$data);
}
function get_fulldata_lotid($id)
{
	...
	if($current_rescode)
	{
		...
		$data['ac_incomes']=$this->Entry_model->get_pending_incomes_by_tempcode($current_rescode);
		if($current_res->pay_type!='Outright')
			{
				$data['loan_data']=$loan_data=$this->reservation_model->get_eploan_data($current_rescode);
				if($loan_data){
				$data['ac_incomes2']=$this->Entry_model->get_pending_incomes_by_tempcode($loan_data->loan_code);
				}
			}
...
//        }
        ...
        $data['income_entry'] = $this->paymentvoucher_model->get_incomedata_by_entryid($entry_id);
 		...
 		$data['income_entry'] = $income_dataset= $this->paymentvoucher_model->get_incomesByEntryid($entry_id);
//			print_r($income_dataset);
		if($income_dataset->amount <= 0){
		//	print_r($income_dataset);
					 $update_data = array(
				...
				$this->db->where('id',$entry_id)->update('re_prjacincome',$update_data);
     			 update_jurnal_entry_insert($income_dataset->income_type,$income_dataset->id,date('Y-m-d'));
