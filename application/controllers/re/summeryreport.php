...
public function summery_report()
	...
	//	create_branchaccluntlist();
		...
		$data['prjlist']=$prjlist=$this->report_model->get_all_project_summery($this->session->userdata('branchid'));
					$this->load->view('re/report/summery_main',$data);
	...
	function collection_summery()
	{
		...
		$data['enddate']=$enddate=$year.'-'.$month.'-31';
			if($prjlist)
			{
				...
				{
					$advance[$prjraw->prj_id]=$this->report_model->Summery_advance_sum_this_month($prjraw->prj_id,$stdate,$enddate);
					$zep[$prjraw->prj_id]=$this->report_model->Summery_loan_payment_month($prjraw->prj_id,$stdate,$enddate,'ZEP');
				$nep[$prjraw->prj_id]=$this->report_model->Summery_loan_payment_month($prjraw->prj_id,$stdate,$enddate,'NEP');
				$epb[$prjraw->prj_id]=$this->report_model->Summery_loan_payment_month($prjraw->prj_id,$stdate,$enddate,'EPB');
				$epb[$prjraw->prj_id]=0;
					...
					$this->load->view('re/report/summery_collection',$data);	
	...
	function collection_summery_date_range()
	{
		if ( ! check_access('view_report'))
		{
		...
		$data['prjlist']=$prjlist=$this->report_model->get_all_project_summery($branchid);
		$data['month']=$month='';
		$data['stdate']=$stdate=$this->uri->segment(6);
		$data['enddate']=$enddate=$this->uri->segment(7);
			if($prjlist)
			{
				foreach($prjlist as $prjraw)
				{
		...
		$data['enddate']=$enddate=$year.'-'.$month.'-31';
			if($prjlist)
			{
				...
				//$epb[$prjraw->prj_id]=$this->report_model->Summery_loan_payment_month($prjraw->prj_id,$stdate,$enddate,'EPB');
					...
					$this->load->view('re/report/summery_collection_excel',$data);	
	...
	function balance_summery()
	{
		...
		$data['enddate']=$enddate=$year.'-'.$month.'-31';
			if($prjlist)
			{
				foreach($prjlist as $prjraw)
				{
					$lots[$prjraw->prj_id]=$this->report_model->summery_pending_stock($prjraw->prj_id);
					$soldlots[$prjraw->prj_id]=$this->report_model->summery_reserved_stock($prjraw->prj_id,$enddate);
					$advancelist[$prjraw->prj_id]=$this->report_model->Summery_reservation_balance_month($prjraw->prj_id,$stdate,$enddate);
					$zeplist[$prjraw->prj_id]=$this->report_model->Summery_loan_balance_month($prjraw->prj_id,$stdate,$enddate,'ZEP');					$eplist[$prjraw->prj_id]=$this->report_model->Summery_loan_balance_month($prjraw->prj_id,$stdate,$enddate,'NEP');
					$epblist[$prjraw->prj_id]=$this->report_model->Summery_loan_balance_month($prjraw->prj_id,$stdate,$enddate,'EPB');
					...
					$this->load->view('re/report/summery_balance',$data);	
		...
		$data['enddate']=$enddate=$year.'-'.$month.'-31';
			if($prjlist)
			{
				foreach($prjlist as $prjraw)
				{
					...
					$this->load->view('re/report/summery_balance_excel',$data);	
