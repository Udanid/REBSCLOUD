		...
		if($prjlist)
		{
			...
			{ $end=$todate;
				...
				while($start<=$end)
				{
					...
					$targetdata=$this->report_model->get_month_target($prjraw->prj_id,$month2,$year,$prjraw->officer_code);
					if($targetdata)
...
//	echo $yeararr[0]."ssss".$this->session->userdata('fy_end');
	    ...
	    foreach($prjlist as $prjraw)
		{
			...
			else
			{
				...
				while($start<=$end)
				{
					...
					$transferlist[$prjraw->prj_id][$month2]=$this->report_model->get_profittransfer_data_period_project_summery($prjraw->prj_id,$stdate,$enddate)	;
	...
	{
		//Ticket No:2537 Added By Madushan 02/04/2021
		$data['type']=$type=$this->uri->segment(5);
		$data['branchid']=$branchid=$this->uri->segment(4);
		if($type == '07')
		{
			$reservedblocks = NULL;
			$data['date']=$date=$this->uri->segment(6);
			$data['prj_id']=$prj_id=$this->uri->segment(7);
			$data['dataset']=$this->report_model->get_all_block_details_list($branchid,$prj_id,$date);
			if($data['dataset']){
			foreach($data['dataset'] as $result)
			{
				$lot_id = $result->lot_id;
				$reservedblocks[$lot_id] = $this->report_model->get_reserved_block_details($lot_id);
			}
			$data['reservedblock'] = $reservedblocks;
			}
			// foreach($data['dataset'] as $row)
			// {
			// 	if($reservedblocks[$row->lot_id])
			// 		echo $reservedblocks[$row->lot_id]->res_code.'<br>';
			// 	//echo $reservedblocks[$row->lot_id]->full_name.'<br>';
			// 	//echo $row->lot_id.'<br>';
			// }
			// exit;
			$this->load->view('re/report/all_block_details',$data);
		}
		...
		else if($type=='06')
		{
			...
			if($prjlist){
				foreach($prjlist as $prjraw)
				{
					...
					$zepreservation[$prjraw->prj_id]=$this->report_model->allloans_befor($prjraw->prj_id,$enddate);
					//print_r($zepreservation[$prjraw->prj_id]);
					if($zepreservation[$prjraw->prj_id])
					{
						foreach($zepreservation[$prjraw->prj_id] as $resraw)
					...
					$this->load->view('re/report/collection_allloans_all',$data);
			}
		...
		$this->load->view('re/report/collection_summery_data.php',$data);
		}
	}
