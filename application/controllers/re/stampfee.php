...
class Stampfee extends CI_Controller {
	 ...
	 */
	 function __construct() {
		...
		$this->load->model("deedtransfer_model");
			$this->load->model("stampfee_model");
	...
	public function showall()
	{
		...
		if ( ! check_access('view_stampfee'))
		{
			...
			$data['prjlist']=$this->stampfee_model->get_all_project_confirmed($this->session->userdata('branchid'));
				$data['deedlist']=$this->stampfee_model->get_stampfee_request_list();
				$data['paidlist']=$this->stampfee_model->get_stampfee_paidlist_list();
		...
		//	$data['deedlist']=$this->deedtransfer_model->get_deedtranfers_by_type();
				...
				$this->load->view('re/stampfee/stampfee_main',$data);
	...
	function get_blocklist($id)
{
	...
	$data['lotlist']=$this->stampfee_model->get_stampfeeoaidlist_list($id);
	$this->load->view('re/stampfee/blocklist',$data);
}
	...
	public function get_fulldata()
	{
		...
		$data['chargedata']=$this->reservation_model->get_charge_data($current_rescode);
		
		if($current_rescode)
		{
			$data['requestdata']=$this->stampfee_model->get_stampfee_request($current_rescode);
		...
		$this->load->view('re/stampfee/payment_data',$data);
	...
	public function add_request()
	{
		...
		$id=$this->stampfee_model->add();
		
		$this->common_model->add_notification('re_charge_stampfee','Stamp Fee Payment Request','re/stampfee',$id);
	...
	public function approved_request()
	{
		...
		$id=$this->stampfee_model->approved_request();
		
		$this->common_model->delete_notification('re_charge_stampfee','Stamp Fee Payment Request','re/stampfee',$id);
		$this->session->set_flashdata('msg', 'Stamp Fee Payment Request Successfully Inserted ');
		$this->logger->write_message("success",'Stamp Fee Payment Request successfully Inserted');
		redirect("re/stampfee/showall");
	...
	public function delete()
	{
		...
		$id=$this->stampfee_model->delete_request($this->uri->segment(4));
		
		$this->common_model->delete_notification('re_charge_stampfee',$this->uri->segment(4));
	...
	public function report()
	{
		...
		//	$data['deedlist']=$this->deedtransfer_model->get_deedtranfers_by_type();
				...
				$this->load->view('re/stampfee/stampfee_report',$data);
	...
	function report_fulldata()
	{
		...
		$data['blocklist']=$blocklist=$this->stampfee_model->get_all_blocklist($prj_id);
		//$data['chargedata']=$this->reservation_model->get_charge_data($current_rescode);
		
		
		$this->load->view('re/stampfee/report_data',$data);
	}
	function report_fulldata_excel()
	{
		...
		$this->load->view('re/stampfee/report_data_excel',$data);
	}
