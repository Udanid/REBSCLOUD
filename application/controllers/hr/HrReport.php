	...
	function view_expense_summery()
	{
		...
		if($employee_list)
		{
				...
				$emp_payroll=$this->hrreport_model->get_emp_payroll($employee_id,date("m",strtotime($from_date)),date("Y",strtotime($from_date)));
						...
						if($emp_allowances){
							foreach ($emp_allowances as $key => $value) {
								if($value->allowance=='Vehicle Rent'){// vaild only for winrose.
									...
									$allowance_emp_sum=$value->amount;
					...
					if($fuel_allowance_payment)
					{
						foreach ($fuel_allowance_payment as $key => $value) {
							$fual_expence_emp_sum=$value->total_amount_payable;
						}
					}
					$phone_bill_payment=$this->hrreport_model->get_emp_phone_bill_payment($employee_id,date("m",strtotime($from_date)),date("Y",strtotime($from_date)));
					if($phone_bill_payment)
					{
						foreach ($phone_bill_payment as $key => $value) {
							$phone_bill_emp_sum=$value->bill_value;
						}
					}else{
						$phone_bill_emp_sum=0;
					}
					...
					$phone_bill_emp=$phone_bill_emp+$phone_bill_emp_sum;
	        ...
	        $fromyear = date("Y",strtotime($from_date));
				...
				$employee_a[$employee_id]['vehicle_rent_emp']=$vehicle_rent_emp;//pass to view each
				...
				$employee_a[$employee_id]['phone_bill_emp']=$phone_bill_emp;
			...
			$viewData['basic_salary_tot']=$basic_salary_tot;//pass to view tot
...
//$from_date2=$from_date;
			...
			$emp_payroll=$this->hrreport_model->get_emp_payroll($employee_id,date("m",strtotime($from_date)),date("Y",strtotime($from_date)));
				...
				if($fuel_allowance_payment)
				{
					foreach ($fuel_allowance_payment as $key => $value) {
						...
						//$phone_bill_emp=$phone_bill_emp+100;
					}
				}
				$phone_bill_payment=$this->hrreport_model->get_emp_phone_bill_payment($employee_id,date("m",strtotime($from_date)),date("Y",strtotime($from_date)));
				if($phone_bill_payment)
				{
					foreach ($phone_bill_payment as $key => $value) {
						$phone_bill_emp=$phone_bill_emp+$value->bill_value;
					}
				}
        ...
        array_push($phone_bill,$phone_bill_emp);
...
//$emp_payroll=$this->hrreport_model->get_emp_payroll(25,'08',date("Y",strtotime('2018-06-01')));
    //print_r($emp_payroll);
		...
		$frommonth = date("m",strtotime($from_date));
		$fromyear = date("Y",strtotime($from_date));
		$viewData['emp_payroll'] =$emp_payroll=$this->hrreport_model->get_emp_payroll($employee_id,$frommonth,$fromyear);
