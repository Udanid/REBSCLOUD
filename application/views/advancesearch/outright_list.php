...
<div id="page-wrapper">
 <div class="main-page">
  <div class="table">
      ...
      <h3 class="title1">Outright Settlement Data</h3>
     			
      <div class="widget-shadow">
        <div class="form-title">
		<h4>Outright Search List
       <span style="float:right"> <a href="javascript:expoet_excel()"> <i class="fa fa-file-excel-o nav_icon"></i></a>
</span></h4>
	</div>
               <div role="tabpanel" class="tab-pane fade  active in" id="home" aria-labelledby="home-tab" >
               <br>
             	  <form data-toggle="validator" id="myexportform" method="post" action="<?=base_url()?>advancesarch/outrightlist_excel"  enctype="multipart/form-data">
              ...
              <BR>
                    <div class=" widget-shadow bs-example" data-example-id="contextual-table" > 
                        ...
                        <table class="table"> <thead> <tr> <th>Reservation Code</th><th>Branch Name</th><th>Project Name</th><th>Lot Number</th> <th>Customer Name </th><th>Reserve Date</th><th>Estimated Price</th><th>Discount</th><th>Sale Value</th><th>MDP</th><th>Paid Amount</th></tr> </thead>
                      ...
                      <?  $brnes=0;$brndis=0;$brnsale=0; $brnmdp=0;$brnpaid=0;$brnbmdp=0;
                       ...
                       <tr > 
                        <td scope="row"><?=$row->res_code?></td><td scope="row"><?=get_branch_name($row->branch_code)?></td><td> <?=$row->project_name ?></td><td> <a href="javascript:load_lotinquary('<?=$row->lot_id?>','<?=$row->prj_id?>')" ><?=$row->lot_number ?>-<?=$row->plan_sqid ?></a></td> <td><?=$row->first_name ?> <?=$row->last_name ?></td> <td><?=$row->res_date?></td>
                        ...
                        <td align="right"><?=number_format($row->down_payment,2)?></td>
								...
								$prjbmdp=$prjbmdp+($row->discounted_price-$row->down_payment);
								$prj_id=$row->prj_id;
								...
								$brnbmdp=$brnbmdp+($row->discounted_price-$row->down_payment);
