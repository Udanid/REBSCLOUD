       ...
       <span style="float:right"><!-- <a href="javascript:expoet_excel()"> <i class="fa fa-file-excel-o nav_icon"></i></a>-->
              ...
              <BR>
                 ...
                 <div class="tableFixHead">
                        <table class="table"> <thead> <tr class="active"> <th>Loan Code</th><th>Branch Name</th><th>Project Name</th><th>Lot Number</th> <th>Customer Name </th><th>Rebate Date</th><th>Loan Amount</th><th>Paid Amount</th><th>Repay Capital</th><th>Repay Int</th><th> Status </th></tr> </thead><tbody>
                       ...
                       <tr > 
                        <td scope="row"><?=$row->loan_code?></td><td scope="row"><?=get_branch_name($row->branch_code)?></td><td> <?=$row->project_name ?></td><td> <a href="javascript:load_lotinquary('<?=$row->lot_id?>','<?=$row->prj_id?>')" ><?=$row->lot_number ?>-<?=$row->plan_sqid ?></a></td> <td><?=$row->first_name ?> <?=$row->last_name ?></td> <td><?=$row->apply_date?></td>
                         <td><?=number_format($row->loan_amount,2)?></td>
                           <td><?=number_format($row->paid_capital,2)?></td>
                        <td><?=number_format($row->repay_capital,2)?></td>
                         <td><?=number_format($row->repay_int,2)?></td>
                       
                        <td align="right"><?=$row->status ?></td> 
                           </tr> 
                        
                                <?
								$prjes=$prjes+$row->loan_amount;
								$prjdis=$prjdis+($row->paid_capital);
								$prjsale=$prjsale+$row->repay_capital;
								$prjmdp=$prjmdp+$row->repay_int;
										$prj_id=$row->prj_id;
								$brnes=$brnes+$row->loan_amount;
								$brndis=$brndis+($row->paid_capital);
								$brnsale=$brnsale+$row->repay_capital;
								$brnmdp=$brnmdp+$row->repay_int;
