		...
		<table class="table" style="border-collapse:collapse;" border="1">
			...
			<thead>
				...
				<tr bgcolor="LightGray" >
					...
					<th colspan="<?php echo count($allowance_list)+count($insentive_list); ?>" style="text-align: center">Allowances</th>

					<th rowspan="2">Gross Salary</th>

					<th rowspan="2">No Pay</th>

					<th colspan="<?php echo count($deduction_list)+count($other_deduction_list)+1; ?>" style="text-align: center">Deductions</th>
				...
				<tr bgcolor="LightGray">
					...
					foreach($insentive_list as $insentive){
						?>

						<th><?php echo $insentive->allowance; ?></th>
					...
					foreach($deduction_list as $deduction){
						...
						<th><?php echo $deduction->deduction; ?></th>
					...
					foreach($other_deduction_list as $other_deduction){

						?>

						<th><?php echo $other_deduction->deduction; ?></th>
			...
			<tbody>
					...
					foreach($payroll_list as $row){
						...
						<tr class="<?php echo ($c<0) ? 0 : ($c++ % 2 == 1) ? 'info' : ''; ?>">
							...
							foreach($insentive_list as $insentive){
								...
								$emp_payroll_insentive_details = $ci->employee_model->get_employee_payroll_insentive_list($row->payroll_master_id, $row->emp_record_id, $insentive->id);

								if(count($emp_payroll_insentive_details) > 0){

									$insentive_amount = $emp_payroll_insentive_details['amount'];
								...
								<td id="" class="text-right"><?php echo number_format($insentive_amount,2, '.', ''); ?></td>
							...
							foreach($other_deduction_list as $deduction2){
								$other_deduction = 0;
								$emp_payroll_deduction_details2 = $ci->employee_model->get_employee_payroll_otherdeduction_list($row->payroll_master_id, $row->emp_record_id, $deduction2->id);
								if(count($emp_payroll_deduction_details2) > 0){

										$other_deduction = $emp_payroll_deduction_details2['amount'];

									}?>
									<td class="medical text-right"><?php echo number_format($other_deduction,2, '.', ''); ?></td>
									<?
				...
				<tr bgcolor="LightGray">
					...
					foreach($allowance_bra as $allowance){
					...
					<th class="total-phone_bill text-right">Phone chargers</th>
...
$(document).ready(function () {
	...
	$('tr').each(function () {
		...
		$(this).find('.phone_bill').each(function () {
			var combat = $(this).text();
			if (!isNaN(combat) && combat.length !== 0) {
				phonebillsum =phonebillsum+ parseFloat(combat);
			}
		});
	...
	$('.total-phone_bill', this).html(phonebillsum.toFixed(2));
...
$("#generate_excel_icon").click(function (e) {
	...
	this.download =date+"_payroll.xls";
