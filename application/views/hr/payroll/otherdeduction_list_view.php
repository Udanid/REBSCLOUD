...
?>
<h4>Other Deduction  Payment Details: <?php echo $month."/".$details['year']; ?><span style="float:right; color:#FFF" ><a href="javascript:close_edit('<?php echo $details['id']; ?>')"><i class="fa fa-times-circle"></i></a></span></h4>
...
<table class="table">
                  <thead>
                    <tr>
                      ...
                      <th>Employee No</th>
                      <th>Name</th>
                      <th>Other Deduction Amount(Rs)</th>
                      <th></th>
                    </tr>
                  </thead>
                  <tbody>
                    <?php
				    if($other_deduct_list){
				  	  $c = 0;
					  $count = 1;
				  	  $ci =&get_instance();
					  $ci->load->model('employee_model');
                  	  foreach($other_deduct_list as $row){
