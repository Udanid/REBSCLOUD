...
?>
<h4>Go In & Go Out Report: <?php echo "From - ".$from_date.", To - ".$to_date.", Branch - ".ucfirst($branch); ?><span style="float:right; color:#FFF" ><a href="javascript:close_edit()"><i class="fa fa-times-circle"></i></a></span></h4>
<div name="print_div" id="print_div" class="table widget-shadow" style="background-color:transparent;">
  <h4 style="background-color: transparent; color: black; text-align: center; display:none;" name="print_title" id="print_title"> <br>Go In & Go Out Report: <?php echo "From - ".$from_date." To - ".$to_date.", Branch - ".ucfirst($branch); ?></h4>
      ...
      <thead
        <colgroup>
		  ...
		  while ($date_display <= $to_date){
		    $check_attendance = $ci->employee_model->check_go_in_go_out_attendance_for_date($date_display);
        ...
        <tr bgcolor="LightGray">
		  ...
		  $date_display = $from_date;
		  while ($date_display <= $to_date){?>
		    <th><?php echo date('d-M', strtotime($date_display));; ?></th>
		    <?php
			$date_display = date('Y-m-d', strtotime($date_display . ' +1 day'));
		  } ?>
        </tr>
				   ...
				   ?>
					<tr>
					  ...
					  while ($date_display <= $to_date){
						...
						$emp_attendance = $ci->employee_model->get_emp_go_in_go_out_attendance_report($date_display,$employee->id);
						if(date('D', strtotime($date_display)) == 'Sun'){ ?>
							<td style="text-align: center;"></td>
						<?php
						}else if(count($emp_attendance)>0){ ?>
							<td style="text-align: center; background-color:lightgreen;"><?php foreach($emp_attendance as $emp_attendance_row){ echo date('H:i', strtotime($emp_attendance_row->go_out)).' - '.date('H:i', strtotime($emp_attendance_row->go_in)).'<br>'; } ?></td>
						<?php
						}else{ ?>
...
<script>
	...
	$("#generate_excel_icon").click(function (e) {
        ...
        this.href = result;
        this.download = "go_in_go_out_report.xls";
		$('#print_title').hide();
		$('#print_icon').show();
		$('#generate_excel_icon').show();
		$('body').html(restorepage);
