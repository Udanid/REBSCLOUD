...
<div id="page-wrapper">
 ...
 <div class="tableFixHead"> 
                        ...
                        <table class="table"> <thead> <tr class="active"> <th>Customer Number</th><th>NIC</th> <th>Customer Type</th><th>Name</th> <th>Mobile </th><th>Land Phone </th><th>Work Phone </th> <th>E mail</th> <th>Status</th><th></th></tr> </thead><tbody> 
                      <? if($datalist){$c=0;
                          foreach($datalist as $row){?>

                        <tr class="<? echo ($c<0) ? 0 : ($c++ % 2 == 1) ? 'info' : ''; ?>">
                        <th scope="row"><?=$row->cus_number?></td><th><?=$row->id_number?></th><td><?=ucfirst($row->cus_type)?></td><td><?=strtoupper($row->first_name) ?> <?=strtoupper($row->last_name) ?></td> <td><?=$row->mobile?></td><td><?=$row->landphone?></td><td><?=$row->workphone?></td>
                        <td><?=$row->email?></td>
                        <td><?=$row->status ?></td>
                        <td align="right"><div id="checherflag">
                        <? if(! check_access('all_branch')){
								if($row->cus_branch==$this->session->userdata('branchid')){?>
                                   ...
                                   <? }?>
                                <? }else{?>
                                    <a  href="javascript:viewCustomer('<?=$row->cus_code?>')" title="View"><i class="fa fa-eye nav_icon icon_green"></i></a>
                                 <? }?>
                        <? }else{?>
                        		<a  href="javascript:viewCustomer('<?=$row->cus_code?>')" title="View"><i class="fa fa-eye nav_icon icon_green"></i></a>
                        		<a  href="javascript:check_activeflag('<?=$row->cus_code?>')" title="Edit"><i class="fa fa-edit nav_icon icon_blue"></i></a>
