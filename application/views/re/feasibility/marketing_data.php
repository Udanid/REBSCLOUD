<script>
 function check_this_market()
 {
	...
	var mydptot=parseFloat(document.getElementById('percentage1').value)+parseFloat(document.getElementById('percentage2').value)+parseFloat(document.getElementById('percentage3').value)+parseFloat(document.getElementById('percentage4').value)+parseFloat(document.getElementById('percentage5').value)+parseFloat(document.getElementById('percentage6').value);

	if(mydptot!=parseFloat(document.getElementById('epsales').value))
	{
		//alert(parseFloat(document.getElementById('epsales').value));
	document.getElementById('totaldp').value=myeptot;
	}
	else
 ...
 <div class="  widget-shadow" data-example-id="basic-forms">
	...
	<div class="form-body">
     <div class="form-group form-horizontal" style="float:left"><label class="col-md-3 control-label">Outright sales (%)</label>
										...
										<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
										<span class="help-block with-errors" ></span></div>
                                         <label class="col-sm-3 control-label" >Interest Free Months</label>
										<div class="col-sm-3 has-feedback"><input type="number" class="form-control" id="int_freetime" onBlur="calculate_tot(this.value)"   value="<?=$details->int_freetime?>" name="int_freetime"   pattern="[0-9]+([\.][0-9]{0,2})?"  data-error=""  required>
										<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
										<span class="help-block with-errors" ></span>
                                        </div>
                                        <label class="col-sm-3 control-label" >Discount Rate</label>
										<div class="col-sm-3 has-feedback"><input type="number" step="0.01" class="form-control" id="prj_discount"  value="<?=$details->prj_discount?>" name="prj_discount"   pattern="[0-9]+([\.][0-9]{0,2})?"  data-error=""  required>
										<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
										<span class="help-block with-errors" ></span>
                                        </div></div>
   ...
   <div class="form-body">
      <table class="table"> <thead> <tr> <th >ID</th> <th >Down Payment</th>  <th >Percentage</th></tr> </thead>
                                  ...
                                  <tr> <td><?=$count?></td>
                                   ...
                                   <td>Total</td>
                                    <td> <div class="form-group has-feedback" ><input type="text"   class="form-control" name="totaldp"  id="totaldp"   pattern="[0-9]+([\.][0-9]{0,2})?"  required="required" value="<?=$tot?>" data-error="Total must euql to easy payment sales value "> <span class="glyphicon form-control-feedback" aria-hidden="true"></span>
										<span class="help-block with-errors" ></span>
									</div></td>
                                    </tr></table>
	...
	<div class="form-title">
	<h4>Financing Data - Borrowing Rates(%)</h4>
	</div>
	<div class="form-body  form-horizontal">

      <div class="form-group" style="float:left"><label class="col-sm-4 control-label">Bank Borrowing Rate</label>
										<div class="col-sm-4 has-feedback"><input type="text" class="form-control"   id="land_bank"  value="<?=$details->land_bank ?>" name="land_bank" pattern="[0-9]+([\.][0-9]{0,2})?"    required>
                                        <span class="glyphicon form-control-feedback" aria-hidden="true"></span>
										<span class="help-block with-errors" ></span></div>
                                        <label class="col-sm-1 control-label" ><!--Other--></label>
										<div class="col-sm-3 has-feedback"><input type="hidden" class="form-control" id="other_rate" onBlur="calculate_tot(this.value)"   value="<?=$details->other_rate?>" name="other_rate"   pattern="[0-9]+([\.][0-9]{0,2})?"  data-error=""  required>
										<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
										<span class="help-block with-errors" ></span></div>
                  </div>
                                         <div class="form-group" style="float:left">
                                           <label class="col-sm-4 control-label" >Sales Tax &nbsp;&nbsp;&nbsp;</label>
   										<div class="col-sm-4 has-feedback"><input type="text" class="form-control" id="sales_tax" onBlur="calculate_tot(this.value)"   value="<?=$details->sales_tax?>" name="sales_tax"   pattern="[0-9]+([\.][0-9]{0,2})?"  data-error=""  required>
   										<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
   										<span class="help-block with-errors" ></span></div>
                                           <label class="col-sm-1 control-label"><!--Branch Rate--></label>
										<div class="col-sm-3 has-feedback"><input type="hidden" class="form-control"   id="branch_rate"  value="<?=$details->branch_rate  ?>" name="branch_rate" pattern="[0-9]+([\.][0-9]{0,2})?"    required>
                                        <span class="glyphicon form-control-feedback" aria-hidden="true"></span>
										<span class="help-block with-errors" ></span></div>
                                        </div>
     ...
     <div class="form-title">
	<h4>Financing Data - Loan Facilities Proposed</h4>
	</div>
	<div class="form-body  form-horizontal">
    <table class="table">
    <thead><tr><th>Request Month</th><th>Amount</th></tr></thead>
    <? for($i=1; $i<=6; $i++ ){?>
		<tr><td><?=$i?></td><td><div class="form-group has-feedback"><input type="text" class="form-control"  onBlur="calculate_tot(this.value)"   <? if($bankloan[$i]){?>value="<?=$bankloan[$i]->amount?>"<? }else{?>value="0" <? }?> name="landbankmonth<?=$i?>"  id="landbankmonth<?=$i?>"   pattern="[0-9]+([\.][0-9]{0,2})?"  data-error=""  required></div></th></tr>
	<? }?>
    <td>
    </table>
								...
								<table class="table"> <thead> <tr> <th >Period</th><th  >Percentage</th></tr> </thead>
                                  ...
                                  <tr> <td><?=$raw->timerange?></td>
                                   ...
                                   <?  $count++; }
                                    ...
                                    <? }?><tr>
                                   <td>Total</td>
                                    <td> <div class="form-group has-feedback" ><input type="text"  value="<?=$tot?>"   class="form-control" name="totalep" id="totalep"  max="100"  pattern="[0-9]+([\.][0-9]{0,2})?" min="100" data-error="Total Ep rates Must euql to 100" required="required"> <span class="glyphicon form-control-feedback" aria-hidden="true"></span>
										<span class="help-block with-errors" ></span>
									</div></td>
                                    </tr></table>
