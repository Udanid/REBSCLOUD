...
<style type="text/css">
 ...
 <?
   $heading2=' EP (NEP) Arrears Report as at '.$date;
   ...
   <div class="form-title">
    <h4><?=$heading2?> 
     ...
     <div class="table-responsive bs-example widget-shadow"   style="max-height:400px; overflow:scroll" >
      ...
      <tr>
        <th colspan="26" style="text-align:center;"><h4>EP (NEP) Arrears Report as at <?=$date?></h4></th>
      </tr>
     ...
     <th >Repayment Period (Months)</th>
      ...
      <th>No of rental due</th>
          <th >Arrears Amount as at <?=$sartdate?></th>
          <th>Current Month Due Amount</th>
          <th>Total Due for current month</th>
          <th> Payment Date</th>
         <th >Recieved Amount Current month</th>
         <th >Arrears Amount as at <?=$date?></th>
         <th>Last Payment Date</th>
          <th >No of rental in arreras</th>
  ...
  <?
      ...
      foreach($transferlist[$prraw->prj_id] as $raw){
        ...
        $arrearsmonth=0;$thismonthpay=0;$arrinscount=0;$creditmonth=0;$arrtot=0;
                $startarrarstot=0; $current_installment = 0;
        $fullint=0;
        ...
        <tr class="<?=$myclass?>"><td><a href="javascript:get_loan_detalis('<?=$raw->loan_code?>')"><?=$raw->unique_code?></a></td><td><?=$raw->lot_number?></td>
         ...
         <td align="right"><?=number_format($fullint,2)?></td>
          ...
          <td align="right"><?=$due_rentals[$raw->loan_code]->no_of_due?></td>
         <td align="right"><?=number_format($startarrarstot,2)?></td>
          <td align="right">
                ...
                $current_installment = $current_month_details[$raw->loan_code]->tot_instalment;
                ?>
                <?}?>
                </td>
            <td align="right"><?=number_format(($current_installment + $startarrarstot),2)?></td>
             <td>
     ...
     $finttot=$finttot+$fullint; 
        ...
        $balinttot=$balint+$balinttot; 
         ...
         $instalment_tot = $instalment_tot + $raw->montly_rental;
           $due_tot = $due_tot + ($current_installment + $startarrarstot);
            if(isset($current_month_details[$raw->loan_code]->tot_instalment))
