...
{
			window.open( "<?=base_url()?>re/report_excel/get_provition/"+prjid+'/'+month);
	
}
function load_printscrean3(prjid,fromdate,todate)
{
			window.open( "<?=base_url()?>re/report_excel/get_provition_daterange/"+prjid+'/'+fromdate+'/'+todate);
...
<div class="row">
   ...
   <tr class="success" style="font-weight:bold"><td colspan="9"  align="center"><?=$details[$prj_id]->project_name?></td></tr>
      ...
      <th >Subtask</th><th >Payee Name</th><th >Narration</th><th >Payment Date </th><th>CHQ / Voucher No</th><th >%</th>
    ...
    <? 
        ...
        <td align="right"><?=number_format($raw->estimate_budget,2)?></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
        <? if($currentlist[$raw->id]){foreach($currentlist[$raw->id] as $payraw){?>
			<tr><td></td><td></td>
              <td align="right"><?=number_format($payraw->amount,2)?></td>
          <td><?=$payraw->subtask_name?></td>
           <td ><?=$payraw->name?></td>
            <td><?=$payraw->narration?></td>
