          ...
          <? $this->load->view("includes/flashmessage");?>
               ...
               <br>
                    ...
                    <div class=" widget-shadow bs-example" data-example-id="contextual-table" > 
                        ...
                        <table class="table"> <thead> <tr> <th>Reservation Code</th><th>Loan Code</th><th>Land Details</th> <th>Customer Name </th><th>Reserve Date</th><th style="text-align:right;">Down Payment</th><th style="text-align:center;">Purchase Type</th><th>Create By </th><th>Confirm By </th></tr> </thead>
                      <? if($datalist){$c=0;
                          foreach($datalist as $row){
							  if($row->pay_type!='Outright'){
								  
								  $type=get_actual_loantype($row->loan_code)?>
                      
                        <tbody> <tr class="<? echo ($c<0) ? 0 : ($c++ % 2 == 1) ? 'info' : ''; ?>"> 
                        <th scope="row"><?=$row->res_code?> <th scope="row"><?=$row->unique_code?></th><td><a href="javascript:load_lotinquary('<?=$row->lot_id?>','<?=$row->prj_id?>')" > <?=$row->project_name ?>&nbsp; <?=$row->lot_number ?>-<?=$row->plan_sqid ?></a></td> <td> <a  href="javascript:viewCustomer('<?=$row->cus_code?>')" title="View Customer Info"><?=$row->first_name ?> <?=$row->last_name ?></a></td> <td><?=$row->res_date?></td>
...
<form name="deletekeyform">  <input name="deletekey" id="deletekey" value="0" type="hidden">
            ...
            $("#complexConfirm").confirm({
                ...
                confirm: function(button) {
					...
					var code=1
                    window.location="<?=base_url()?>re/reservation/delete_loan/"+document.deletekeyform.deletekey.value;
                },
			 ...
			 $("#complexConfirm_advdelete").confirm({
              ...
              $("#complexConfirm_confirm").confirm({
                ...
                confirm: function(button) {
                    ...
                    window.location="<?=base_url()?>re/reservation/confirm_loan/"+document.deletekeyform.deletekey.value;
                },
                cancel: function(button) {
                    button.fadeOut(2000).fadeIn(2000);
                   // alert("You aborted the operation.");
