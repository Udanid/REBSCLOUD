		   ...
		   for(i=1; i<count; i++)
			...
			document.getElementById('fulltot').value= maintot.toLocaleString();
       		...
       		<a href="javascript:call_confirm_budget('<?=$prjdata->prj_id?>')"><span class="label label-success">Confirm Budget</span></a>
                    ...
                    <table class="table"><tr><th>Task Name</th><th>Last Estimation</th><th>Payments</th><th>New Budget</th><th>Total</th></tr>
                    <? $total=0; $count=1;if($details){ foreach ($details as $raw){
                        ...
                        <input type="number" class="form-control" step="0.01" id="newbdg<?=$count?>"  name="newbdg<?=$count?>"  value="<?=round($raw->estimate_budget,2)?>"  min="<?=$raw->tot_payments?>" onkeyup="calculete_brdgettot()" onblur="calculete_brdgettot()"  data-error=""  required="required" ><span class="glyphicon form-control-feedback" aria-hidden="true"></span>
