...
$( function() {
    $( "#paydate" ).datepicker({dateFormat: 'yy-mm-dd' ,minDate: '<?=$this->session->userdata("current_start")?>',
      ...
      <div class="widget-shadow">
          <ul id="myTabs" class="nav nav-tabs" role="tablist"> 
           ...
           <li role="presentation"  <? if(!$this->session->flashdata('tab')){?> class="active"<? }?>><a href="#profile" role="tab" id="profile-tab" data-toggle="tab" aria-controls="profile" aria-expanded="true">Monthly Instalment</a></li> 
             <li role="presentation"  <? if($this->session->flashdata('tab')=='history'){?> class="active"<? }?>><a href="#history" role="tab" id="history-tab" data-toggle="tab" aria-controls="history" aria-expanded="true" onClick="chosenActivate()">Payment History</a></li> 
           ...
           <div id="myTabContent" class="tab-content scrollbar1" style="padding:5px;">
            ...
            <? $this->load->view("includes/flashmessage");?>
                ...
                <div role="tabpanel" class="tab-pane fade <? if(!$this->session->flashdata('tab')){?>active in<? }?>" id="profile" aria-labelledby="profile-tab"> 
                       ...
                       <form data-toggle="validator" method="post" action="<?=base_url()?>re/eploan/pay_rental" enctype="multipart/form-data">
                       <div class="row">
                    ...
                    <option value="<?=$row->loan_code?>" ><?=$loanarr?>- <?=$row->id_number?> <?=$row->last_name ?> </option>
                    <? }?>
