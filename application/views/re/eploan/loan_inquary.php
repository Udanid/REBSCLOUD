   ...
   function loadcurrent_block(id)
{
 if(id!=""){
					...
					$( "#myloanlist" ).load( "<?=base_url()?>re/eploan/get_project_loan_inquiry/"+id );
...
function load_loanlist(id)
{
 if(id!=""){
							 ...
							 $('#myloanlist').delay(1).fadeIn(600);
    					    document.getElementById("myloanlist").innerHTML='<img src="<?=base_url()?>media/images/loading.gif"  class="loadinggif">'; 
					$( "#myloanlist" ).load( "<?=base_url()?>re/eploan/get_lot_loan_inquiry/"+id );
...
function loan_fulldata(id)
{
 ...
 if(id!=""){
					...
					$( "#plandata" ).load( "<?=base_url()?>re/eploan/get_loanfulldata/"+id );
 ...
 <form data-toggle="validator" method="post" action="<?=base_url()?>re/reservation/add_advance" enctype="multipart/form-data">
