...
{
			window.open( "<?=base_url()?>re/eploan/print_loaninquary/"+id );
	
}
function load_printscrean2(id)
{
	
			window.open( "<?=base_url()?>re/eploan/excel_loaninquary/"+id );
...
function get_ledgercard(lotid)
{
 ...
 <div class="form-title">
                <h4>Loan Details
                    <span style="float:right"> 
                        <a href="javascript:load_printscrean2('<?=$details->loan_code?>')"> <i class="fa fa-file-excel-o nav_icon"></i></a> 
                        <a href="javascript:load_printscrean1('<?=$details->loan_code?>')"> <i class="fa fa-print nav_icon"></i></a>
                        <a href="javascript:get_repayment_shedule('<?=$details->loan_code?>')" title="Repayment Schedule"><i class="fa fa-calendar nav_icon "></i></a>
                        <a href="javascript:get_ledgercard('<?=$details->res_code?>')" title="Ledger Card"><i class="fa fa-file nav_icon "></i></a>
                        <a href="javascript:get_bloackdetails('<?=$details->prj_id?>','<?=$details->lot_id?>')" title="Block Detail"><i class="fa fa-sitemap nav_icon icon_blue"></i></a>
                        <a href="javascript:get_followuphistory('<?=$details->cus_code?>')" title="Follow Up"><i class="fa fa-phone nav_icon "></i></a>
                    </span>
                </h4>
							</div>
 <div class=" widget-shadow bs-example" data-example-id="contextual-table" > 
				  ...
				  if($rebate){
				  	$arreascap = $arreascap - ($rebate->balance_capital-$rebate->credit_int)-$rebate->new_discount;
				  	$arreasint = $arreasint - $rebate->int_paidamount - $rebate->int_release;
				  }
				...
				<?  }?>
                 <h3> </h3>
					  ...
					  // print_r($totset);
					   ...
					   $paidint=$totset->totpaidint;
                       if($rebate)
					   {
						   if($totset->totpaidcap>0)
						   $paidcap=($rebate->balance_capital - $rebate->credit_int - $rebate->new_discount)+$totset->totpaidcap;
						   else
						   $paidcap=($rebate->balance_capital - $rebate->credit_int - $rebate->new_discount);

						if($totset->totpaidint>0)
						   $paidint=$rebate->int_paidamount+$totset->totpaidint+$rebate->int_release;
						else
						  $paidint=$rebate->int_paidamount+$rebate->int_release;
					   ...
					   ?>
                        ...
                        <th scope="row">Balance Capital</th><td  align="right"> <?=number_format($details->loan_amount-$paidcap,2)  ?> &nbsp; </td><th  >Balance Interest</th><td align="right"><?=number_format($totint-$paidint,2) ?></td> <th  >Balance Total</th><td align="right"><?=number_format(($totint+$details->loan_amount)-($paidint+$paidcap),2)  ?></td>
                         </tr> 
                         <tr class="table-bordered info"> 
...
<tr><th>Paid percentage </th><td> <div class="task-info">
                         ...
                         <tr class="warning"> 
                        <th scope="row">Arrears Rental</th><td align="right"> <?=number_format($arreascap+$arreasint,2)  ?> &nbsp; </td><th  align="right">Delay Interest</th><td align="right"><?=number_format(get_loan_date_di($details->loan_code,date('Y-m-d')),2)?></td> <th  align="right">Arrears Instalments</th><td align="right"><?=round(($arreascap+$arreasint)/$details->montly_rental,2) ?></td>
                         </tr> 
                          <tr class="warning"> 
                        <th scope="row">Arrears Capital</th><td align="right"> <?=number_format($arreascap,2)  ?> &nbsp; </td><th  align="right">Arrears Interest</th><td align="right"><?=number_format($arreasint,2)?></td><th  align="right">Total Arrears Amount</th><td align="right"><?=number_format($arreascap+$arreasint+get_loan_date_di($details->loan_code,date('Y-m-d')),2)  ?></td>
                         </tr> 
                   ...
                   <th  >Credit Capital</th><td align="right"><?=number_format($creditcap,2) ?></td>    <th >Credit Interest</th><td align="right"><?=number_format($creditint,2) ?></td>  <th scope="row">Credit Balance</th><td align="right"> <?=number_format($creditcap+$creditint,2)  ?> &nbsp; </td>
                        ...
                        <th scope="row">Delay Interest  Payments</th><td  align="right"> <?=number_format($totset->totpaiddi,2)  ?> &nbsp; </td><th  align="right"></th><td></td><td colspan="2"></td>
                         </tr> 
 ...
 <table class="table table-bordered"><tr><th>ID</th><th>Delay Interest</th><th> Paid Capital</th><th> Paid Interest</th><th> Total Pay Amount</th><th>Balance Capital</th><th> Balance Interest</th><th>Payment Date</th><th>Receipt No </th><th>Receipt Date </th></tr>
                ...
                <? }}?></table>
				   ...
				   if($rebate)
					   {?>
                         <table class="table table-bordered"><tr><th>Instalment</th><th>Rental</th><th>Due Date</th><th>Delay Interest</th><th> Paid Capital</th><th> Paid Interest</th><th> Credit Interest</th><th>Balance Capital</th><th> Balance Interest</th><th>Payment Date</th><th>Receipt No </th><th>Additional Discount</th></tr>
                       <tr>
                       <td>Early Settlment</td><td>-</td><td>-</td><td><?=number_format($rebate->delay_int ,2) ?></td>
                       	<td><?=number_format((($rebate->balance_capital-$rebate->credit_int)-$rebate->new_discount) ,2) ?></td>
                  
                       <td><?=number_format($rebate->int_paidamount,2) ?></td>
                       <td><?=number_format($rebate->credit_int ,2) ?></td>
                       <td>-</td><td>-</td>
                       <td><?=$rebate->apply_date?></td>
                        <td><?=$rebate->rct_no?></td>
                        <td><?=number_format($rebate->new_discount,2)?></td>
                       </tr>
                       <? }?>
                         ...
                         <? if($rshceduls){?>
                             ...
                             <div class="form-title">
                            	...
                            	<div class="form-body  form-horizontal" >
                               <table class="table table-bordered"><tr><th>Reshedule Date</th><th>Loan Capital</th><th>Paid Capital</th><th>Paid Interest</th><th>Credit Interest</th><th>Arrears Interest</th><th> Initial Rate</th><th>Instalments</th><th>Reshceduled Capital </th></tr>
                ...
                <td><?=number_format($row->loan_paidcrint ,2) ?></td>
                  <td><?=number_format($row->arrears_int  ,2) ?></td>
