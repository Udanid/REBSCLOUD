...
foreach($lotlist[$rw->plan_sq] as $row){	
	?> <tbody> <tr class="<? echo ($c<0) ? 0 : ($c++ % 2 == 1) ? 'info' : ''; ?>"> 
                        ...
                        <input type="text"    style="width:50px; padding:3px;" class="form-control" id="lot_number<?=$count?>" name="lot_number<?=$count?>" value="<?=str_pad($row->lot_number, 2, "0", STR_PAD_LEFT)?>" required="required"></td>
                         ...
                         <td> <div class="form-group has-feedback" ><input type="text" class="form-control number-separator" name="price<?=$count?>"  onblur="calculate_salestot('<?=$count?>')"  id="price<?=$count?>" value="<?=$row->price_perch?>" required><span class="glyphicon form-control-feedback" aria-hidden="true"></span>
                        </div></td>
