...
function load_fulldetails()
{
	...
	//  alert('block_val')
	 if(prj_id!="")
	 {
		...
		//	alert("<?=base_url()?>re/roi_report/roi_data/"+prj_id+'/'+todate)
			 $('#fulldata').delay(1).fadeIn(600);
    		  document.getElementById("fulldata").innerHTML='<img src="<?=base_url()?>media/images/loading.gif"  class="loadinggif">'; 
		 	  $( "#fulldata").load( "<?=base_url()?>re/roi_report/roi_data/"+prj_id+'/'+todate);
		}
		 else
		 {
		  
	 		 document.getElementById("checkflagmessage").innerHTML='Please Select Date  report'; 
					$('#flagchertbtn').click(); 
		 }
	 }
	 else
	 {
		 prj_id=0;
		  document.getElementById("checkflagmessage").innerHTML='Please Project to generate report'; 
					$('#flagchertbtn').click(); 
