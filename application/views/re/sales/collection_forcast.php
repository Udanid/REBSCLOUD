...
function load_fulldetailsf()
{
	 ...
	 var monthf=document.getElementById("monthf").value;
	  var yearf=document.getElementById("yearf").value;
	   var loantypef=document.getElementById("loantypef").value;
	
	 if(prj_idf!="" & monthf!="" & yearf!='' & loantypef!='')
	 {
		// alert("<?=base_url()?>re/sales/get_forcast/"+prj_idf+'/'+monthf+'/'+yearf+'/'+loantypef)
		 $('#fulldataf').delay(1).fadeIn(600);
    		  document.getElementById("fulldataf").innerHTML='<img src="<?=base_url()?>media/images/loading.gif"  class="loadinggif">'; 
		 	  $( "#fulldataf").load( "<?=base_url()?>re/sales/get_forcast/"+prj_idf+'/'+monthf+'/'+yearf+'/'+loantypef);
...
function format_val(obj)
{
	...
	obj.value=parseFloat(a).toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
       ...
       <form data-toggle="validator" method="post" action="<?=base_url()?>re/sales/add_forcast"  enctype="multipart/form-data">
            ...
            <div class="form-body">
                <div class="form-inline">
                    ...
                    <div class="form-group" id="blocklist">
                     ...
                     <div class="form-group" id="blocklist">
                        <select  name="loantypef" id="loantypef" class="form-control" >
