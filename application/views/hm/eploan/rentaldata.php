...
function check_value()
{
	...
	if(parseFloat(amount)==0)
	{
		document.getElementById("checkflagmessage").innerHTML='Pay Amount Couldnt Be Zero'; 
					 ...
					 document.getElementById("payment").value="";
							...
							if($retruncharge>0){?>
                             <div class="form-group"><label class="col-sm-3 control-label">Cheque Retrun Charge</label>
                                        ...
                                        <? }?>
                                   <table class="table"><tr><th>Instalment</th><th>Rental</th><th>Due Date</th><th>Delay Interest</th><th>Paid DI</th><th>Paid Total</th><th>Credit Balance</th><th  style="color:#F30">DI Waive Off</th><th>Payment</th></tr>
                         <? $delaintot=0;$inslist="";$tot=0;$thisdelayint=0;
				  $delayrentaltot=0; $arieasins=0; if($ariastot){foreach($ariastot as $row)
				  {
					  $loopstartdate=$row->deu_date;
					  $date1=date_create($row->deu_date);
						if($row->pay_date>$row->deu_date)
						{
						$date1=date_create($row->pay_date);
						$loopstartdate=$row->pay_date;
						}
					...
					if($delay_date >0)
					{
						$count=ceil($delay_date/30);
						//echo $count;
						 $instalmetdi=0;$mydi=0;
						 $end_date2=$loopstartdate;
						for($i=1; $i<=$count; $i++)
						{
						$dalay_int=($instalmetdi+floatval($row->tot_instalment-$paidtotals))*floatval($details->delay_interest)/(100);				//	$end_date=date('Y-m-d',strtotime('+'.intval($i).' months',strtotime($row->deu_date)));
 					//	$ispaid=is_paid_this_perod_di($details->loan_code,$row->id,$end_date2,$end_date);
					//	if($ispaid)
					//	$ispaid=$ispaid;
					//	else $ispaid=0;
				//	echo $dalay_int.'-'.$end_date2.'/'.$end_date.'-'.$row->instalment.'--'.$ispaid.'<br>';
					//	if($ispaid==0)
					//	{
							
						$instalmetdi=$instalmetdi+$dalay_int;
					//	}
					//	$end_date2=date('Y-m-d',strtotime('+'.intval($i).' months',strtotime($end_date2)));
						//else
						//$instalmetdi=$dalay_int;
						
						//$instalmetdi=$instalmetdi+$dalay_int;
					
						}
						$dalay_int=$instalmetdi;
					...
					// $delaintot= $delaintot+$dalay_int-$row->balance_di;
					 //echo $row->balance_di;
					 $currentdi=round($dalay_int+$row->uptodate_balancedi,2);
					 if($currentdi<0)
					  $currentdi=0;
				...
				//	$thispayment=$currentdi;
					// echo  $details->montly_rental;	
					 if($delay_date >0)
					{
					  $tot=$thispayment+$tot;
					}
