...
//alert(id);
		...
		$.ajax({
            ...
            data: {table: 'hm_epresale', id: id,fieldname:'resale_code' },
            success: function(data) {
				...
				else
				{
					...
					$( "#popupform" ).load( "<?=base_url()?>hm/eploan/edit_resale/"+code+"/"+id );
				}
            }
        });
}
...
<div id="page-wrapper">
 <div class="main-page">
  ...
  <div class="table">
      ...
      <h3 class="title1">Loan Reprocess</h3>
     			
      <div class="widget-shadow">
          <ul id="myTabs" class="nav nav-tabs" role="tablist"> 
           <li role="presentation" class="active">
          <a href="#home" id="home-tab" role="tab" data-toggle="tab" aria-controls="home" aria-expanded="false">Reprocess List</a></li> 
          <li role="presentation">
          <a href="#profile" role="tab" id="profile-tab" data-toggle="tab" aria-controls="profile" aria-expanded="true">Reprocess Loan</a>
          </li> 
          ...
          <? $this->load->view("includes/flashmessage");?>
               ...
               <br>
                    ...
                    <div class=" widget-shadow bs-example" data-example-id="contextual-table" > 
                        ...
                        <table class="table"> <thead> <tr> <th>Loan Code</th><th>Article Details</th> <th>Customer Name </th><th>Loan Amount</th><th>Paid Amount</th><th>Reprocess Date</th><th>Remark</th><th>Apply By </th><th>Confirm By </th></tr> </thead>
                      <? if($datalist){$c=0;
                          foreach($datalist as $row){?>
						
                        <tbody> <tr class="<? echo ($c<0) ? 0 : ($c++ % 2 == 1) ? 'info' : ''; ?>"> 
                        <th scope="row"><a  href="javascript:get_loan_detalis('<?=$row->loan_code?>')" title="Loan Full Details"><?=$row->loan_code?></a></th><td> <?=$row->project_name ?>&nbsp; <?=$row->lot_number ?>-<?=$row->plan_sqid ?></td> <td><?=$row->first_name ?> <?=$row->last_name ?></td>
                        <td align="right"><?=number_format($row->loan_amount,2)?></td> 
                        <td align="right"><?=number_format($row->paid_capital,2)?></td>
                         <td align="right"><?=$row->apply_date?></td>
                          <td ><?=$row->remark?></td>
                ...
                <div role="tabpanel" class="tab-pane fade " id="profile" aria-labelledby="profile-tab"> 
                    <p>	 
                  <?  $this->load->view("hm/eploan/resale_new"); ?>
                   </p> 
                </div>
...
<form name="deletekeyform"><input name="deletekey" id="deletekey" value="0" type="hidden">
            ...
            $("#complexConfirm").confirm({
                ...
                confirm: function(button) {
					...
					var code=1
                    window.location="<?=base_url()?>hm/eploan/delete_resale/"+document.deletekeyform.deletekey.value;
                },
              ...
              $("#complexConfirm_confirm").confirm({
                ...
                confirm: function(button) {
                    ...
                    window.location="<?=base_url()?>hm/eploan/confirm_resale/"+document.deletekeyform.deletekey.value;
                },
                cancel: function(button) {
                    button.fadeOut(2000).fadeIn(2000);
                   // alert("You aborted the operation.");
