...
function load_projectdata(code)
{
	...
	//alert()
		...
		$('#month').delay(1).fadeOut(600);
		$('#fromdateid').delay(1).fadeOut(600);
		$('#todateid').delay(1).fadeOut(600);
		}
		else
		{
				$('#month').delay(1).fadeIn(600);
				$('#fromdateid').delay(1).fadeIn(600);
				$('#todateid').delay(1).fadeIn(600);
		}
	}
	else if(code=='04' || code=='02')
	{
	  ...
	  if(code!='')
	  {
			  ...
			  if(prj_id!="")
				...
				$('#fulldata').delay(1).fadeIn(600);
    			  document.getElementById("fulldata").innerHTML='<img src="<?=base_url()?>media/images/loading.gif"  class="loadinggif">'; 
				   ...
				   if (fromdate!='' & todate!='')
				   {
					    $( "#fulldata").load( "<?=base_url()?>hm/report_daterange/get_provition/"+prj_id+'/'+fromdate+'/'+todate);
				   }
				   else
				    $( "#fulldata").load( "<?=base_url()?>hm/report/get_provition/"+prj_id+'/'+month);
		 ...
		 if(code=='02')
		 {
				 ...
				 if(prj_id!="")
				 {
    				  ...
    				  document.getElementById("fulldata").innerHTML='<img src="<?=base_url()?>media/images/loading.gif"  class="loadinggif">'; 
		 			  	 if (fromdate!='' & todate!='')
				  		 {
						    $( "#fulldata").load( "<?=base_url()?>hm/report_daterange/get_budget/"+prj_id+'/'+fromdate+'/'+todate);
				  		 }
				  		 else
				  		  $( "#fulldata").load( "<?=base_url()?>hm/report/get_budget/"+prj_id+'/'+month);
		 ...
		 if(code=='04')
		 {
				 ...
				 if(prj_id!="")
				 {
						$('#fulldata').delay(1).fadeIn(600);
    				  document.getElementById("fulldata").innerHTML='<img src="<?=base_url()?>media/images/loading.gif"  class="loadinggif">'; 
		 			  $( "#fulldata").load( "<?=base_url()?>hm/report/get_finance/"+prj_id);
		 ...
		 if(code=='03'){
			// var prj_id= document.getElementById("prj_id").value;
    		  ...
    		  document.getElementById("fulldata").innerHTML='<img src="<?=base_url()?>media/images/loading.gif"  class="loadinggif">';
			   if (fromdate!='' & todate!='')
				  		 {
						    $( "#fulldata").load( "<?=base_url()?>hm/report_daterange/get_unbudget/"+fromdate+'/'+todate+'/'+branchid);
				  		 }
				  		 else
				  		   $( "#fulldata").load( "<?=base_url()?>hm/report/get_unbudget/"+month+'/'+branchid);
		  ...
		  if(code=='05'){
			...
			// var prj_id= document.getElementById("prj_id").value;
				$('#fulldata').delay(1).fadeIn(600);
    		  document.getElementById("fulldata").innerHTML='<img src="<?=base_url()?>media/images/loading.gif"  class="loadinggif">'; 
		 	  $( "#fulldata").load( "<?=base_url()?>hm/report/get_finance_summery/"+branchid+'/'+branchid);
		 }
		  if(code=='06'){
			  //alert(branchid)
			// var prj_id= document.getElementById("prj_id").value;
				$('#fulldata').delay(1).fadeIn(600);
    		  document.getElementById("fulldata").innerHTML='<img src="<?=base_url()?>media/images/loading.gif"  class="loadinggif">'; 
		 	  $( "#fulldata").load( "<?=base_url()?>hm/report/get_provition_summery/"+branchid+'/'+branchid);
		 }
	  }
	  else
	  { 
...
{
			 document.getElementById("projectdata").innerHTML='<img src="<?=base_url()?>media/images/loading.gif"  class="loadinggif">'; 
			  $( "#projectdata").load( "<?=base_url()?>hm/report/get_branch_projectlist/"+id);
            ...
            <div class="form-body">
                <div class="form-inline">
                  ...
                  <div class="form-group" id="blocklist">
                        <select  name="collection" id="collection" class="form-control" onChange="load_projectdata(this.value)">
                         <option value="">Report Type</option>
                          <option value="04">Finance Cost Report</option>
                            <option value="05">Finance Cost Summary Report</option>
                        <option value="01">Provision Report</option>
                           <option value="06">Provision Summary Reprt</option>
                        <option value="02">Budget Report</option>
                        <option value="03">Unbudgeted  Expense Report</option>
...
<form name="deletekeyform">  <input name="deletekey" id="deletekey" value="0" type="hidden">
            ...
            $("#complexConfirm").confirm({
                ...
                confirm: function(button) {
					...
					var code=1
                    window.location="<?=base_url()?>hm/project/delete/"+document.deletekeyform.deletekey.value;
                },
              ...
              $("#complexConfirm_confirm").confirm({
                ...
                confirm: function(button) {
                    ...
                    window.location="<?=base_url()?>hm/lotdata/confirm_price/"+document.deletekeyform.deletekey.value;
                },
                cancel: function(button) {
                    button.fadeOut(2000).fadeIn(2000);
                   // alert("You aborted the operation.");
