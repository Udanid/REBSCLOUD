...
{
			window.open( "<?=base_url()?>hm/report_excel/get_profit_all/"+branchid+"/"+prjid+"/"+month );
	
}
function load_printscrean3(branchid,prjid,fromdate,todate)
{
			window.open( "<?=base_url()?>hm/report_excel/get_profit_all_daterange/"+branchid+"/"+prjid+"/"+fromdate+"/"+todate );
...
<div class="row">
   ...
   <div class="form-title">
		<h4><?=$heading2?> 
       <span style="float:right"> <a href="javascript:load_printscrean1('0','<?=$month?>')"> <i class="fa fa-print nav_icon"></i></a>      <? if(isset($fromdate) & isset($todate)){ ?>
       <a href="javascript:load_printscrean3('<?=$branchid?>','0','<?=$fromdate?>','<?=$todate?>')"> <i class="fa fa-file-excel-o nav_icon"></i></a>
       <? }else{?>
           <a href="javascript:load_printscrean2('<?=$branchid?>','0','<?=$month?>')"> <i class="fa fa-file-excel-o nav_icon"></i></a>
