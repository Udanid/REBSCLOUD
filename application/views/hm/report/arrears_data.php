...
function expoet_excel(month,branch,year)
{
				...
				window.open( "<?=base_url()?>hm/report/arreas_data_excel/"+month+'/'+branch+'/'+year);
}
function expoet_excel2(month,branch,year)
{
		
		
				window.open( "<?=base_url()?>hm/report/arreas_data_excel_ins/"+month+'/'+branch+'/'+year);
}
...
function add_followup_detalis(id,date)
{
 ...
 if(id!=""){
	 ...
	 $('#popupform').delay(1).fadeIn(600);
					...
					$( "#popupform" ).load( "<?=base_url()?>hm/eploan/get_followupdata_popup/"+id+"/"+date);
...
<div class="row">
   ...
   <div class="form-title">
		<h4><?=$heading2?> 
       <span style="float:right"> <a href="javascript:expoet_excel('<?=$month?>','<?=$branchid?>','<?=$year?>')"> <i class="fa fa-file-excel-o nav_icon"></i></a>
</span></h4>
	</div>
   ...
   <tr>  <th >Arrears Amount</th> <th>Instalment</th> <th>Total Receivables</th> <th >Arrears Amount</th> <th>Instalment</th> <th>Total Receivables</th><th>Opening</th><th>Closing</th><th>Variance</th></tr>
	...
	<?
			...
			?>
        <?  if($transferlist[$prraw->prj_id]){//e//cho $prraw->prj_id;
	...
	//  $fulltot=$fulltot+$prjexp;
     ...
     <div class="table-responsive bs-example widget-shadow"   style="max-height:400px; overflow:scroll" >
       ...
       <span style="float:right"> <a href="javascript:expoet_excel2('<?=$month?>','<?=$branchid?>','<?=$year?>')"> <i class="fa fa-file-excel-o nav_icon"></i></a>
</span></h4>
	</div>
