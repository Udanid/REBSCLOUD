...
<div class="row">
   ...
   <div class="form-title">
	...
	</div><input type="hidden" id="rptdate" value="<?=$todate?>">
	...
	if($dataset){ 
		...
		foreach($dataset as $prjraw){
			...
			{
				 $intid=hm_get_first_advance($prjraw->temp_code);// this function is in reaccount_helper
				if($intid==$prjraw->id)
			...
			$line_tot=$adv+$cap_epb+$cap_nep+$cap_zep+$int+$di+$other+$nra;
			 ...
			 <tr class="info" style="font-weight:bold"><td><?=$current_name?> Total</td><td></td><td></td><td></td>
