...
<div class="row">
    ...
    <tr><th>Stamp Fee amount</th><td><?=number_format($chargedata->stamp_duty ,2)?></td></tr>
     ...
     <table class="table"> <thead> <tr> <th>Payment Date</th><th>Charge Type </th><th>Amount</th><th>Receipt No</th> <th>Status </th></tr> </thead>
                      <? if($charge_payment){$c=0;$rctlist=''; $paydate=''; $payamount=0;
                          foreach($charge_payment as $row){ if($row->chage_dis=='Stamp Duty')
 ...
 <? }else {?>
  <div class=" widget-shadow " data-example-id="basic-forms"> 
   ...
   <form data-toggle="validator" method="post" action="<?=base_url()?>hm/stampfee/add_request" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="cus_code" id="cus_code" value="<?=$current_res->cus_code?>">
                         <input type="hidden" name="full_amount" id="full_amount" value="<?=$chargedata->stamp_duty?>">
                          ...
                          <div class="form-body ">
										 ...
										 <div class="col-sm-3  has-feedback "><input type="number"  max="<?=$payamount?>" step="0.01"class="form-control" name="paid_amount" id="paid_amount"  required="required" /></div>
                                         <span class="glyphicon form-control-feedback" aria-hidden="true"></span>
										<span class="help-block with-errors" ></span>
                                         </div> 
                                        
                                         <div class="col-sm-3  has-feedback "><input type="text" class="form-control" name="need_date" id="need_date"  required="required" /></div> <div class="col-sm-3" ><button type="submit" class="btn btn-primary " >Request Payment</button></div></div>
