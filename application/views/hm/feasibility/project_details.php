...
// alert("ssss");
 ...
 <? $this->load->view("includes/flashmessage");?>
                    ...
                    <select name="prj_type" id="prj_type" class="form-control">
                      <? if($prj_type){
                        ...
                        foreach ($prj_type as $key => $value) {?>
                          <option value="<?=$value->prjtype_id?>" <? if($details->prj_typeid==$value->prjtype_id){echo "selected";}?>><?=$value->short_code?> - <?=$value->prjtype_name?></option>
                    <?    }
                      }?>
                  ...
                  <textarea name="ownership" id="ownership" class="form-control"><?=$details->ownership?></textarea>
                  </div>
							...
							<div class="form-body form-horizontal">
                                    ...
                                    <div class="form-group"><label class="col-sm-3 control-label">Land Extend(Perch)</label>
										<div class="col-sm-3"><input type="text" class="form-control"   id="land_extend"  value="<?=$details->land_extend?>" name="land_extend" pattern="[0-9]+([\.][0-9]{0,2})?"    readonly="readonly"  required></div>
									...
									<label class="col-sm-3 control-label">Salable Area</label>
										<div class="col-sm-3 has-feedback"><input type="text" class="form-control" id="selable_area"  value="<?=$details->selable_area?>"  name="selable_area"   pattern="[0-9]+([\.][0-9]{0,2})?"  readonly="readonly" data-error=""  required>
										<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
										<span class="help-block with-errors" ></span></div>
									</div> -->
                  ...
                  <div class="col-sm-3">
                    <input type="number" step="1" name="units" id="units" class="form-control" value="<?=$details->numof_units?>" required>
                    </div>
                  </div>
                ...
                <? if($details->owenership_type!="client_property"){?>
								...
								<div class="form-body">
                                   ...
                                   <input type="text" class="form-control" id="purchase_price"  value="<?=$details->purchase_price?>" name="purchase_price" pattern="[0-9]+([\.][0-9]{0,2})?" placeholder="Land Extend In purch"  required>
										</div>
                  <div class="form-group has-feedback" >
										<label class="control-label">Owner Expected Price</label>
										<input type="text" class="form-control" name="expect_price"  value="<?=$details->expect_price?>" id="expect_price" pattern="[0-9]+([\.][0-9]{0,2})?"   placeholder="Land Extend In Arc" data-error="Invalid number" required>
                                        <span class="glyphicon form-control-feedback" aria-hidden="true"></span>
										<span class="help-block with-errors" ></span>
									</div>

									<div class="form-group has-feedback" ><label class="control-label">Market  Price</label>
                                   <input  type="text"  pattern="[0-9]+([\.][0-9]{0,2})?"   value="<?=$details->market_price?>" class="form-control"  onBlur="calculate_tot(this.value)"  id="market_price" name="market_price"  placeholder="Perch Price"   required>
										  <span class="glyphicon form-control-feedback" aria-hidden="true"></span>
										</div>
									</div>
									<br>
...
?>
							...
							<div class="form-body form-horizontal">
                <div class="form-group"><label class="col-sm-3 control-label">Development Type</label>
                  <div class="col-sm-6 has-feedback">
                  <select name="dev_type" id="dev_type" class="form-control">
                    <? if($development_type){

                      foreach ($development_type as $key => $value) {?>
                        <option value="<?=$value->dp_typeid?>" <? if($details->dp_typeid==$value->dp_typeid){echo "selected";}?>><?=$value->dp_typename?></option>
                  <?    }
                    }?>
 ...
 <? if($details->status=='PENDING'){?>
                      ...
                      <?  }if($fsbstatus && $details->owenership_type!="client_property"){?>
                  <a href="<?=base_url()?>hm/feasibility/generete_evereport/<?=$this->encryption->encode($prj_id)?>" class="btn btn-success">Generate Evaluation Reports</a>
