...
// echo  $projectprice;
 ?>
<form data-toggle="validator"  method="post" action="<?=base_url()?>hm/feasibility/add_budgut" enctype="multipart/form-data">
								...
								<h4>Project Budget</h4>
					              ...
					              <div class="form-body">
                                   ...
                                   <td style=" padding:0; margin:0" > <?=$subraw->subtask_code?> - <?=$subraw->subtask_name?></td>
                                    ...
                                    <td style=" padding:0; margin:0" > <div class="form-group" style=" padding:0; margin:0" ><input type="text"    class="form-control" <? if($raw->task_id==8 || $raw->task_id==9){?> readonly="readonly"<? }?>  style="text-align:right"   name="amount_val<?=$subraw->subtask_id?>" id="amount_val<?=$subraw->subtask_id?>" value="<?=number_format($subraw->budget,2)?>"  onblur="calculate_subtotal('<?=$raw->task_id?>','<?=$subraw->subtask_id?>')"  required="required"> 
                                   ...
                                   <td style="padding:0; margin:0"> <?=$myraw->subtask_code?> - <?=$myraw->subtask_name?></td>
                                    ...
                                    <td style="padding:0; margin:0"> <div class="form-group"  style=" padding:0; margin:0" ><input type="text"     class="form-control" name="amount_val<?=$myraw->subtask_id?>" <? if($raw->task_id==8 || $raw->task_id==9){?> readonly="readonly"<? }?>  id="amount_val<?=$myraw->subtask_id?>" value="0" onblur="calculate_subtotal('<?=$raw->task_id?>','<?=$myraw->subtask_id?>')" required="required">
                                   ...
                                   <? $subvalue=0; if($myraw->subtask_name=='Purchase Value')$subvalue=$projectprice;
								   if(trim($myraw->subtask_name)=='Commission')$subvalue=$projectprice*0.01;
								   
								   ?>
                                    <td style=" padding:0; margin:0" align="right"> <div class="form-group " style=" padding:0; margin:0; text-align:right"  ><input type="text" style="text-align:right"  class="form-control" name="amount_val<?=$myraw->subtask_id?>"   <? if($raw->task_id==8 || $raw->task_id==9){?> readonly="readonly"<? }?>  value="<?=number_format($subvalue,2)?>"   id="amount_val<?=$myraw->subtask_id?>"  onblur="calculate_subtotal('<?=$raw->task_id?>','<?=$myraw->subtask_id?>')" required="required">
                                    <input type="hidden"  class="form-control" name="amount<?=$myraw->subtask_id?>"   id="amount<?=$myraw->subtask_id?>" pattern="[0-9]+([\.][0-9]{0,2})?"  value="<?=$subvalue?>"  required="required"><span class="glyphicon form-control-feedback" aria-hidden="true"></span>
										<span class="help-block with-errors"  style=" padding:0; margin:0"></span>
									</div></td>
                                     <td> </td></tr>
                                   <?  }}}$count++;
                                   ...
                                   <input type="hidden"  class="form-control" id="sublist<?=$raw->task_id?>" name="sublist<?=$raw->task_id?>" value="<?=$subidlist?>">
                                    ...
                                    <?
                                     if($tasktot==0){
										 if($raw->task_name=='Purchase Price')
									 {
											$tasktot=$projectprice;//+($projectprice*0.01);
									 }
										if($raw->task_name=='Stamp Fees')
										$tasktot=($projectprice*get_rate('Stamp Fee')/100)-1000;
										if($raw->task_name=='Legal Fees')
										$tasktot=($projectprice*get_rate('Legal Fee')/100);
										if($raw->task_name=='Introducer Commission')
										$tasktot=($projectprice*get_rate('Introducer  Commission')/100);
									}
									if($raw->task_id==15 || $raw->task_id==14)
									$tasktot=0;
									?>
                                     <td style=" padding:0; margin:0"><div class="form-group" style=" padding:0; margin:0"  > <input type="text"  class="form-control" id="total_val<?=$raw->task_id?>" name="total_val<?=$raw->task_id?>"   onblur="change_thistot(this.value,'<?=$raw->task_id?>')"  <? if($raw->task_id==15 || $raw->task_id==14) {?> readonly="readonly"<? }?>	value="<?=number_format($tasktot,2)?>" required="required">
