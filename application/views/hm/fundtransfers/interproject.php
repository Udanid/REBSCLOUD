...
function loadcurrent_interftask(id)
{
 ...
 if(id!=""){
					...
					$( "#inner_taskdata" ).load( "<?=base_url()?>hm/projectpayments/get_tasklist/"+id );
					//$( "#totask" ).load( "<?=base_url()?>hm/fundtransfers/get_tasklist/"+id );
...
function loadcurrent_interttask(id)
{
 ...
 if(id!=""){
	 ...
	 if(document.getElementById('interf_prj_id').value==id)
   {
 	...
 	$( "#inner_totask" ).load( "<?=base_url()?>hm/fundtransfers/get_tasklist/"+id );
 ...
 <form data-toggle="validator" method="post" action="<?=base_url()?>hm/fundtransfers/add_inner" enctype="multipart/form-data">
