...
function load_settlment(id)
{
 ...
 if(id!=""){
					...
					$( "#plandata_settlment" ).load( "<?=base_url()?>re/reservation/get_settlments/"+id );
...
function load_amount(value)
{
 ...
 <form data-toggle="validator" method="post" action="<?=base_url()?>re/reservation/add_settlement" enctype="multipart/form-data">
...
<div class=" widget-shadow" data-example-id="basic-forms" style="min-height:400px;"> 
                            ...
                            <? if($searchdata){?>
                          <div class="form-group"><label class="col-sm-3 control-label">Select Resevation</label>
                          <div class="col-sm-3 ">  <select class="form-control" placeholder="Qick Search.."   onchange="load_settlment(this.value)" id="res_code_set" name="res_code_set" >
                    <option value="">Search here..</option>
                    <?    foreach($searchdata as $row){?>
                    <option value="<?=$row->res_code?>"><?=$row->res_code?>-<?=$row->project_name?>-<?=$row->plan_sqid ?>-<?=$row->lot_number ?></option>
                    <? }?>
