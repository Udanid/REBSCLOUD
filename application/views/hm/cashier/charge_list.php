...
function loadcurrent_block(id)
{
 ...
 if(id!="" & rescode!=""){
	...
	//alert("<?=base_url()?>hm/cashier/get_chargedata/"+id+"/"+rescode )
					...
					$( "#plandata" ).load( "<?=base_url()?>hm/cashier/get_chargedata/"+id+"/"+rescode );
...
function load_amount(value)
{
 ...
 <form data-toggle="validator" method="post" action="<?=base_url()?>hm/cashier/add_charges" enctype="multipart/form-data">
...
<div class=" widget-shadow" data-example-id="basic-forms" style="min-height:400px;"> 
                    ...
                    <option value="<?=$row->res_code?>"><?=$row->res_code?> - <?=$row->project_name?> - <?=$row->lot_number ?> - <?=$row->first_name ?> <?=$row->last_name ?></option>
                    <? }?>
             
					</select> </div><label class="col-sm-3 control-label">payment Date</label>
										<div class="col-sm-3 has-feedback" id="paymentdateid"><input  type="text" class="form-control" id="pay_date"    name="pay_date" value="<?=date("Y-m-d")?>"    data-error="" required="required" >
