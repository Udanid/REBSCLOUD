		...
		$.ajax({
            ...
            success: function(data) {
				...
				else
				{
					...
					$( "#popupform" ).load( "<?=base_url()?>hm/reservation/edit_complete/"+id );
				}
            }
        });
}
...
<div id="page-wrapper">
 <div class="main-page">
  ...
  <div class="table">
      ...
      <h3 class="title1">Easy Payment Loan list</h3>
     			
      <div class="widget-shadow">
          <ul id="myTabs" class="nav nav-tabs" role="tablist"> 
           <li role="presentation" class="active">
          <a href="#home" id="home-tab" role="tab" data-toggle="tab" aria-controls="home" aria-expanded="false">Loan List</a></li> 
          ...
          <? $this->load->view("includes/flashmessage");?>
               ...
               <br>
                    ...
                    <div class=" widget-shadow bs-example" data-example-id="contextual-table" > 
                        ...
                        <table class="table"> <thead> <tr> <th>Reservation Code</th><th>Loan Code</th><th>Land Details</th> <th>Customer Name </th><th>Reserve Date</th><th>Down Payment</th><th>Purchase Type</th><th>Create By </th><th>Confirm By </th></tr> </thead>
                      <? if($datalist){$c=0;
                          foreach($datalist as $row){
							  if($row->pay_type!='Outright'){?>
                      
                        <tbody> <tr class="<? echo ($c<0) ? 0 : ($c++ % 2 == 1) ? 'info' : ''; ?>"> 
                        <th scope="row"><?=$row->res_code?> <th scope="row"><?=$row->unique_code?></th><td><a href="javascript:load_lotinquary('<?=$row->lot_id?>','<?=$row->prj_id?>')" > <?=$row->project_name ?>&nbsp; <?=$row->lot_number ?>-<?=$row->plan_sqid ?></a></td> <td> <a  href="javascript:update_customerinfo('<?=$row->cus_code?>')" title="Update Customer Info"><?=$row->first_name ?> <?=$row->last_name ?></a></td> <td><?=$row->res_date?></td>
                        ...
                        <? if($row->pay_type!='EPB'){?>  <a  href="javascript:get_repayment_shedule('<?=$row->loan_code?>')" title="Repayment Schedule"><i class="fa fa-calendar nav_icon icon_green"></i></a><? }?>
                        <? if($row->loan_status=='PENDING'){?>
                             <a  href="javascript:call_confirm('<?=$row->res_code?>')" title="Confirm"><i class="fa fa-check nav_icon icon_green"></i></a>
                              <a  href="javascript:call_delete('<?=$row->res_code?>')" title="Reject Agreement"><i class="fa fa-times nav_icon icon_red"></i></a>
                                    <? }?>
                        </div></td>
                         </tr> 
                        
                                <? }}} ?>
                          </tbody></table> <fieldset class="rating">
    <input type="radio" id="star5" name="rating" value="5" /><label class = "full" for="star5" title="Awesome - 5 stars"></label>
                ...
                <div role="tabpanel" class="tab-pane fade " id="profile" aria-labelledby="profile-tab"> 
                    <p>	  
                  <? // $this->load->view("hm/reservation/advance_payment"); ?>
                   </p> 
                </div>
                <div role="tabpanel" class="tab-pane fade " id="settlment" aria-labelledby="settlment-tab"> 
                    <p>	  
                  <? // $this->load->view("hm/reservation/advance_settlements"); ?>
                   </p> 
                </div>
            </div>
         </div>
...
<form name="deletekeyform">  <input name="deletekey" id="deletekey" value="0" type="hidden">
            ...
            $("#complexConfirm").confirm({
                ...
                confirm: function(button) {
					...
					var code=1
                    window.location="<?=base_url()?>hm/reservation/delete_loan/"+document.deletekeyform.deletekey.value;
                },
			 ...
			 $("#complexConfirm_advdelete").confirm({
              ...
              $("#complexConfirm_confirm").confirm({
                ...
                confirm: function(button) {
                    ...
                    window.location="<?=base_url()?>hm/reservation/confirm_loan/"+document.deletekeyform.deletekey.value;
                },
                cancel: function(button) {
                    button.fadeOut(2000).fadeIn(2000);
                   // alert("You aborted the operation.");
