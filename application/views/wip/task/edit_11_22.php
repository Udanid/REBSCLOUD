<script type="text/javascript">
  ...
  $("#addSubtaskforedit").click(function(){
    $("#tblSubtaskforedit").append('<div class="col-md-12" id="divrow'+rowCount+'">\
      <div class="col-md-2 form_padding" style="margin-left: 13px;">\
      ...
      <div class="col-md-2 form_padding">\
        <div class="form-group has-feedback">\
          <input type="text" class="form-control editholddate'+rowCount+'" name="subtaskedit['+rowCount+'][holdenddate]" placeholder="Resum Date" data-error="" style="display:none" id="editIddate'+rowCount+'">\
        </div>\
      </div>\
      <div class="col-md-1 form_padding">\
        <div class="form-group has-feedback">\
          <a title="Delete"><i class="fa fa-times nav_icon icon_red remove_field" onclick="remove_field('+rowCount+')"></i></a>\
        </div>\
      </div>\
    </div>');
    ...
    <div class="row">
      ...
      <? $checkuser=$this->session->userdata('userid');
        ...
        <div class="col-md-6 form_padding">
          <div class="form-group has-feedback">
            ...
            <input type="number" min="0" class="form-control"name="task_duration" id="task_duration"   placeholder="Duration (Days)" data-error=""  required value="<?=$details->task_duration?>" <?if($flag==0){ ?> readonly <? } ?>>
            <span class="glyphicon form-control-feedback" aria-hidden="true"></span>
            <span class="help-block with-errors"></span>
          </div>
        </div>
      ...
      <div class="col-md-6 form_padding" id="showenddatediv">
        <div class="form-group">
          ...
          <input type="text" name="showenddate" <?if($flag==1){ ?> id="task_start_date" <? } ?> class="form-control" readonly placeholder="Task End Date" value="<?=$details->task_edate?>" <?if($flag==0){ ?> readonly <? } ?>>
        </div>
      </div>
      ...
      <div id="tblSubtaskforedit">
        ...
        <? if($subtaskdetails){
          ...
          foreach ($subtaskdetails as $mainraw){
            ...
            ?>
            <div class="col-md-12">
            <div class="col-md-2 form_padding" style="margin-left: 12px;">
              <div class="form-group has-feedback">
            ...
            <div class="col-md-1 form_padding" style="margin-top: -14px;">
              <div class="form-group has-feedback">
                ...
                <? if($mainraw->hold_status=='Hold'){ ?>
                  <input type="checkbox" class="form-control check_hold" name="subtaskedit[<?= $count ?>][hold]" data-error="" id="chkHoldIdedit<?= $count ?>" onclick="editcheckHold(<?= $count ?>);" value="Hold" checked <?if($subflag==1){ ?> readonly <? } ?> style="margin-top: -3px;">
                <? }else{ ?>
                  <input type="checkbox" class="form-control check_hold" name="subtaskedit[<?= $count ?>][hold]" data-error="" id="chkHoldIdedit<?= $count ?>" onclick="editcheckHold(<?= $count ?>);" value="Hold" <?if($subflag==1){ ?> readonly <? } ?>>
                <? } ?>
              </div>
            </div>
            <div class="col-md-2 form_padding">
              <div class="form-group has-feedback">
                <? if($mainraw->hold_status=='Hold'){ ?>
                 <input type="text" class="form-control editholddate<?= $count ?>" name="subtaskedit[<?= $count ?>][holdenddate]" placeholder="Resum Date" data-error="" id="editIddate<?= $count ?>" value="<?=$mainraw->subt_hold_edate?>" <?if($subflag==1){ ?> readonly <? } ?>>
               <? }else{ ?> 
                <input type="text" class="form-control editholddate<?= $count ?>" name="subtaskedit[<?= $count ?>][holdenddate]" placeholder="Resum Date" data-error="" id="editIddate<?= $count ?>" style="display: none;" <?if($subflag==1){ ?> readonly <? } ?>>
          ...
          <?
            ...
            <div class="col-md-1 form_padding" style="margin-top: -14px;">
              <div class="form-group has-feedback">
                <label for="chkHoldIdedit<?= $count ?>" style="margin-left: 14px;">Hold</label>
                <? if($mainraw->hold_status=='Hold'){ ?>
                  <input type="checkbox" class="form-control check_hold" name="subtaskedit[<?= $count ?>][hold]" data-error="" id="chkHoldIdedit<?= $count ?>" onclick="editcheckHold(<?= $count ?>);" value="Hold" checked <?if($subflag==1){ ?> readonly <? } ?>>
                <? }else{ ?>
                  <input type="checkbox" class="form-control check_hold" name="subtaskedit[<?= $count ?>][hold]" data-error="" id="chkHoldIdedit<?= $count ?>" onclick="editcheckHold(<?= $count ?>);" value="Hold" <?if($subflag==1){ ?> readonly <? } ?> style="margin-top: -3px;">
                <? } ?>
              </div>
            </div>
            <div class="col-md-2 form_padding">
