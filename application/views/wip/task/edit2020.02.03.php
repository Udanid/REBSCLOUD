...
$( function() {
   ...
   function editcheckHold(rowno){
    if ($("#chkHoldIdedit"+rowno).is(":checked"))
    {
      ...
      var task_enddate=$("input[name='showenddate']").val();
      ...
      $("#editIddate"+rowno).datepicker("destroy");
  ...
  $("#addSubtaskforedit").click(function(){
    $("#tblSubtaskforedit").append('<div class="col-md-12 bg-grey-100" style="border:1px solid #ccc; padding:10px 0; margin-bottom:5px;" id="divrow'+rowCount+'">\
      ...
      <div class="col-md-3 form_padding">\
        <div class="form-group has-feedback">\
          <input type="number" class="form-control" name="subtaskedit['+rowCount+'][durarion]" max="'+max_days+'" min="0" placeholder="Duration (days)" data-error="" required id="subtaskdurarion'+rowCount+'" onkeyup="display_checkBox('+rowCount+')" onchange="display_checkBox('+rowCount+')">\
        </div>\
      </div>\
...
// check sub task duration
  ...
  //display Hold chceckbox   Ticket 1036 Bilani 2020-01-01
  function display_checkBox(rowno){
    ...
    if(subt_duration!='' && subt_duration!=0){
      ...
      $("#chkHoldIdedit"+rowno).prop('checked', false); // Unchecks it
      $("#hold_date"+rowno).fadeOut("slow");
            ...
            <select class="form-control chosen-select" required  id="task_assign" name="task_assign" <? if($flag==0){ ?> readonly <? } ?> >
              ...
              <? foreach ($allemployees as $raw){?>
                <? if($details->task_assign == $raw->id){ ?>
                  <option value="<?=$raw->id?>" selected="selected"><?=$raw->initial." ".$raw->surname?></option>
                <? } ?> 
                <? if($flag==1){ ?>                  
                <option value="<?=$raw->id?>" > <?=$raw->initial." ".$raw->surname?></option>
              <? }}?>            
          ...
          <?
            ...
            <div class="col-md-1 form_padding">
              <div class="form-group has-feedback">
                <label for="chkHoldIdedit<?= $count ?>">Hold
                <? if($mainraw->hold_status=='Hold'){ ?>
                  <input type="checkbox" class="form-control check_hold" name="subtaskedit[<?= $count ?>][hold]" data-error="" id="chkHoldIdedit<?= $count ?>" value="Hold" checked <? if($subflag==1){ ?> disabled="disabled" onclick="return false;"<? }else{?>onclick="editcheckHold(<?= $count ?>);"<?} ?>>
                <? }else{ ?>
                  <input type="checkbox" class="form-control check_hold" name="subtaskedit[<?= $count ?>][hold]" data-error="" id="chkHoldIdedit<?= $count ?>" value="Hold" <? if($subflag==1){ ?> disabled="disabled" onclick="return false;"<? }else{?>onclick="editcheckHold(<?= $count ?>);"<? } ?>>
                <? } ?>
                </label>
              </div>
            </div>
