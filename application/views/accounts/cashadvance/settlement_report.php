...
<div class="row">
   ...
   <tr>  <th >IOU/Advance No</th> <th >Cash Advance No</th>
   		<th>Voucher No</th>
   		  <th >Status</th>
   		 <th >Cheque No</th>
		 <th >Requested By</th>
       ...
       <? $fulltot=0;$fulltot2=0;$fulltot3=0;?>
        ...
        <td><?=$raw->adv_code?></td>
         <td><?=$raw->voucher_ncode?></td>
        	<td><?=$raw->status?>
						<? //ticket number 3318 updated by nadee 2021-08-19
						if($raw->status=='PAID'){
						if($raw->payvoucher_id!=""){
							$voucher_statues=check_voucher_statues($raw->payvoucher_id);
         ...
         <td><?=$raw->initial?> <?=$raw->surname?></td>
          <td><?=get_branch_name($raw->branch)?></td>
            <td><?=$raw->project_name?></td>
						...
						<td align="right"><?=number_format($raw->amount-$raw->settled_amount-$raw->refund_amount ,2)?></td>
						<td align="right"><?=$raw->payvoucher_id?></td>
						<td width="20%"><?=$raw->description?></td>
							<td align="right"><? //if($raw->status != 'SETTLED'){ commented by udani- request by thushara,// all settement details listed with this modification
								 if($unsettled_data[$raw->adv_id]){?>
