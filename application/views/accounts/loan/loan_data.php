...
{
  window.open( "<?=base_url()?>accounts/loan/get_loanfulldata/"+id+"/print_loaninquary" );

}
function load_printscrean2(id)
{

  window.open( "<?=base_url()?>accounts/loan/get_loanfulldata/"+id+"/excel_loaninquary");
...
<div class="form-title">
  <h4>Loan Details
    <span style="float:right"> <a href="javascript:load_printscrean2(<?=$loan_id;?>)"> <i class="fa fa-file-excel-o nav_icon"></i></a> <a href="javascript:load_printscrean1(<?=$loan_id;?>)"> <i class="fa fa-print nav_icon"></i></a>
      <a href="javascript:get_payment_shedule('<?=$loan_id;?>')" title="Repayment Schedule"><i class="fa fa-calendar nav_icon "></i></a>
      <a href="javascript:call_details('<?=$loan_id;?>')" title="Other Details"><i class="fa fa-sitemap nav_icon icon_blue"></i></a>
    ...
    if($paydetails){
      ...
      <table class="table " >
        ...
        <tbody>
          ...
          <tr class="table-bordered">
            <th scope="row" >Capital</th><td  align="right"><?=number_format($loan_data->loan_amount,2);?> &nbsp; </td><th scope="row" colspan="3"></th>
          </tr>
