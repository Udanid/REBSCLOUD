...
<script type="text/javascript">
  $(document).ready(function() {
	...
	$('#create_excel').click(function(){
		tableToExcel('tbtable', 'Trial Balance', 'Trial_Balance_<?=FYENDV;?>.xls');
...
// Modification done by udani Date : 12-09-2013
    ...
    <thead>
        <tr>
            <th></th>
            <th align="justify">
                    <p style="font-size:14px;"><?=strtoupper(companyname)?>. <?=date('Y', strtotime('-1 year'))?>-<?=date('Y')?> Trial Balance As of <?=date('M d, Y',strtotime(FYENDV))?></p>
	...
	foreach ($all_ac_ledgers as $ledger_id => $ledger_name)
	{
		...
		echo "<td align=right>";
		$dr_total = $this->Ledger_model->get_dr_total_period($ledger_id,FYSTV,FYENDV);
		if ($dr_total)
		{
		...
		echo "</td>";
		echo "<td align=right>";
		$cr_total = $this->Ledger_model->get_cr_total_period($ledger_id,FYSTV,FYENDV);
		if ($cr_total)
		{
