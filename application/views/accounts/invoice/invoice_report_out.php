...
<?  if($datalist){?>
 ...
 <table class="table"> <thead> <thead> <tr> <th> Date</th> <th>Supplier Name</th>  <th>Invoice Type</th> <th>Invoice Number</th><th>Item Description</th> <th>Invoice Amount</th><th>Paid Amount</th><th>Outstanding Amount</th></tr> </thead>
                      <? $prjname='';$brcode='';  
                      ...
                      <? if($prj_id!='' & $prj_id!=$row->supplier_id){?>
                       <tr class="info" style="font-weight:bold"> 
                        <td scope="row" colspan="5"><?=$prjname?> Supplier Total</td>
                        
                       <td align="right"><?=number_format($suptot,2)?></td>
                               <td align="right"><?=number_format($suppaytot,2)?></td>
                                 <td align="right"><?=number_format($suptot-$suppaytot,2)?></td>
                         </tr> 
                      ...
                      <? $prjes=0;$prjdis=0;$prjsale=0; $prjmdp=0;$suppaytot=0;$suptot=0;
                       ...
                       <tr > 
                        <td scope="row"><?=$row->date?></td><td> <?=$row->first_name?> <?=$row->last_name?></td><td> <?=$row->type ?></td> <td><?=$row->inv_no ?></td><td><?=$row->note ?></td>
                        <td align="right"><?=number_format($row->total,2)?></td>
                         <td align="right"><?=number_format($row->totpay,2)?></td>
                         <td align="right"><?=number_format($row->total-$row->totpay,2)?></td>
                         </tr> 
                        ...
                        <td scope="row" colspan="5"> Total</td>
                              <td align="right"><?=number_format($fulltot,2)?></td>
                                <td align="right"><?=number_format($fullpaytot,2)?></td>
                                 <td align="right"><?=number_format($fulltot-$fullpaytot,2)?></td>
                         </tr> 
