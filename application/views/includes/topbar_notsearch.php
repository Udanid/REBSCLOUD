...
<script type="text/javascript">
  ...
  function loadmycall_hr()
  {
    $("#collapsenewsearch").html("");
    $("#collapsenewsearch").load( "<?=base_url()?>hr/advancesarch/searchpanel/" );
  }
...
$notifylist_user=check_notification_user();//Ticket No:3311 Added By Madushan 2021-08-17
$count = 0;
...
$letternotifi=get_letternotification();
...
<!-- End Section -->
        ...
        <li class="dropdown head-dpdn">
          ...
          <ul class="dropdown-menu drp-mnu">
            <li>
			  <div class="notification_header">
			    <h3>You have <?php echo $count; ?> pending Records  to confirm</h3>
			  </div>
			</li>
            <?php
			if($notifylist){
				foreach($notifylist as $raw){ ?>
					<li>
					  <a href="<?php echo base_url().$raw->module; ?>">
						<div class="notification_desc">
						  <p><?php echo $raw->mycount; ?> Pending <?php echo $raw->notification; ?> </p>
						</div>
						<div class="clearfix"></div>
					  </a>
                    </li>
    ...
    $pending_oic_count = 0;
    $check_oic = $ci->user_model->check_officer_in_charge($userid);
		if(count($check_oic)>0){
			$get_oic_records_for_notificaton = $ci->user_model->get_oic_records_for_notificaton($userid);
			if(count($get_oic_records_for_notificaton) > 0){
				$pending_oic_count = count($get_oic_records_for_notificaton);
			}else{
				$pending_oic_count = 0;
			}
		}else{
		...
		<li class="dropdown head-dpdn">
		  <a href="#" class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="fa fa-user"></i><span class="badge blue"><?php echo $pending_emp_additional_fuel_allowance_list_count+$pending_leave_count+$pending_oic_count; ?></span> </a>
		  <ul class="dropdown-menu drp-mnu">
			<li>
			  <div class="notification_header">
      ...
      if(count($check_oic)>0){ ?>
        <li>
          <div class="notification_header">
        <a href="<?php echo base_url();?>leave/officer_in_charge">Officer in Charge</a><span style="background-color: red;border-radius: 50%; color: white;">&nbsp;&nbsp;<?php echo $pending_oic_count; ?>&nbsp;&nbsp;</span></a>
      </div>
      </li>
      <?php
      }?>
	  ...
	  <ul>
		<li class="dropdown profile_details_drop">
		  <a href="#" class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
			<div class="profile_img">
			  ...
			  <?php
			   ...
			   <div class="user-name">
			    <p><?=$this->session->userdata('display_name')?></p> <!--Ticket No-2502 | Added By Uvini-->
		  ...
		  <ul class="dropdown-menu drp-mnu">
			...
			<?php
			if($employee_details['attendance_type'] == "S"){ ?>
