           ...
           <div id="myTabContent" class="tab-content scrollbar1" style="padding:5px;">
             ...
             <? $this->load->view("includes/flashmessage");?>
                    ...
                    <div class=" widget-shadow bs-example" data-example-id="contextual-table" >
                        ...
                        <table class="table table-bordered"> <thead> <tr> <th >ID</th> <th >Name</th> <th >Rate /Amount</th><th></th></tr>
                      </thead>   <tbody>
                      <? if($datalist){$c=0;
                          foreach($datalist as $row){ if($row->name!='Payment Voucher confirmation'){?>

                      <tr class="<? echo ($c<0) ? 0 : ($c++ % 2 == 1) ? 'info' : ''; ?>">
                        <th scope="row"><?=$row->rate_id?></th> <td><?=$row->name ?></td> <td><?=$row->rate?>&nbsp;<?=$row->rate_type?></td>


                        <td align="right"><div id="checherflag">
                        <a  href="javascript:check_activeflag('<?=$row->rate_id?>')"><i class="fa fa-edit nav_icon icon_blue"></i></a> </div></td>
                         </tr>
<? }  else
				{?>
				<tr class="<? echo ($c<0) ? 0 : ($c++ % 2 == 1) ? 'info' : ''; ?>">
                        <th scope="row"><?=$row->rate_id?></th> <td><?=$row->name ?></td> <td> <div class="form-group has-feedback">
                                    <div class="radio">
                                        <label>
                                          <input type="radio" onChange="update_this_data(this,'<?=$row->rate_id?>')" name="voucher_confimation" value="1" <? if($row->rate==1){?>checked="checked"<? }?>>
                                         Need Payment voucher confirmation
                                        </label>
                                    </div>
                                    <div class="radio">
                                        <label>
                                          <input type="radio" name="voucher_confimation"  onChange="update_this_data(this,'<?=$row->rate_id?>')" value="0" <? if($row->rate==0){?>checked="checked"<? }?> >
                                         Skip voucher confirmation
                                        </label>
                                    </div>
                                </div> </td>
