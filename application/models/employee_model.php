...
class Employee_model extends CI_Model{
	...
	public function add_employee($data_set){
		...
		$data8['emp_id'] = $employeeMasterID;
		$data8['balance'] = 0;
		$this->db->trans_start();
		$this->db->insert('re_commision_staffdeduct', $data8);
		$this->db->trans_complete();
	...
	public function update_employee_details($data_set){
				...
				foreach($epf_etf_details as $epf_etf){
					if($epf_etf->type == "EPF"){
						if($employee_salary->epf == "Y"){
							$epf_employee_contribution = ($basic_salary - $no_pay_deduction_for_epf_etf) * ($epf_etf->employee_contribution/100);
							$epf_employee_contribution =  round($epf_employee_contribution, 2);
							$epf_employer_contribution = ($basic_salary - $no_pay_deduction_for_epf_etf) * ($epf_etf->employer_contribution/100);
							$epf_employer_contribution =  round($epf_employer_contribution, 2);
						}
					}
					if($epf_etf->type == "ETF"){
						if($employee_salary->etf == "Y"){
							$etf_employee_contribution = ($basic_salary - $no_pay_deduction_for_epf_etf) * ($epf_etf->employee_contribution/100);
							$etf_employee_contribution =  round($etf_employee_contribution, 2);
							$etf_employer_contribution = ($basic_salary - $no_pay_deduction_for_epf_etf) * ($epf_etf->employer_contribution/100);
	...
	//	$voucherid=get_vaouchercode('voucherid','PV','ac_payvoucherdata',date('Y-m-d'));
		...
		$data=array(
			...
			'ledger_id' =>'HEDPE62000100',
			'payeename' => 'Cash' ,
			'vouchertype' => '10',
			'paymentdes' => 'Fuel  Allowance  for '.$monthName,
			'amount' => $tot,
	...
	function emp_pendingleaves_in_monthend($insertId)
	{
				...
				while (strtotime($stdate) <= strtotime($date)) {
					...
					if($employee->working_days_per_week=='5' && $pending_records=='true'){
						if(!(date('D', strtotime($stdate)) == 'Sun') && $check_holiday_date=="N"){
							$emp_leave_status = $this->get_emp_leave_status($stdate,$employee->id);
	...
	function add_vauchers_payroll($id)
	{
		...
		if($result){
			...
			if($vehicle_rent!=0){
				...
				$this->add_vouchers($voucherid,'HEDPE61002000','10','Vehicle Rent',number_format($vehicle_rent,2, '.', ''));

			}
			if($allowance_amount!=0){
				$voucherid=$this->paymentvoucher_model->getmaincode('voucherid','PV','ac_payvoucherdata',date("Y-m-d h:i:s"));
				$this->add_vouchers($voucherid,'HEDPE61000200','10','Allowances',number_format($allowance_amount,2, '.', ''));
			}
			if($salary!=0){
				$voucherid=$this->paymentvoucher_model->getmaincode('voucherid','PV','ac_payvoucherdata',date("Y-m-d h:i:s"));
				$this->add_vouchers($voucherid,'HEDPE61000100','10','Salaries',number_format($salary,2, '.', ''));
