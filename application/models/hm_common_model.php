...
class Hm_common_model extends CI_Model {
	...
	function add_notification($tablename,$notification,$module,$recordkey)
	{
		//$tot=$bprice*$quontity; 
		$data=array( 
	...
	{
		$this->db->select('hm_prjfsubtask.*,cm_subtask.subtask_name');
		
		$this->db->where('hm_prjfsubtask.prj_id',$id);
		$this->db->where('hm_prjfsubtask.task_id',$taskid);
		$this->db->join('cm_subtask','cm_subtask.subtask_id=hm_prjfsubtask.subtask_id');
	
		$query = $this->db->get('hm_prjfsubtask'); 
		 if ($query->num_rows >0) {
            return $query->result();
        }
		else
...
{
		$this->db->select('hm_epfollowups.*,hr_empmastr.initial,hr_empmastr.surname');
		$this->db->where('hm_epfollowups.cus_code',$cus_code);
			$this->db->join('hr_empmastr', 'hr_empmastr.id = hm_epfollowups.emp_code','left');
...
function get_resale_payment($res_code)
	{
		$this->db->select('hm_adresalepayment.*,ac_payvoucherdata.status,ac_chqprint.CHQNO');
			$this->db->join('ac_payvoucherdata','ac_payvoucherdata.voucherid=hm_adresalepayment.voucher_code');
		$this->db->join('ac_chqprint',"ac_chqprint.PAYREFNO=ac_payvoucherdata.entryid and ac_payvoucherdata.status='PAID'",'left');
	
		$this->db->where('hm_adresalepayment.resale_code',$res_code);
	...
	{
		$this->db->select('hm_epresalepayment.*,ac_payvoucherdata.status,ac_chqprint.CHQNO');
			$this->db->join('ac_payvoucherdata','ac_payvoucherdata.voucherid=hm_epresalepayment.voucher_code');
		$this->db->join('ac_chqprint',"ac_chqprint.PAYREFNO=ac_payvoucherdata.entryid and ac_payvoucherdata.status='PAID'",'left');
	
		$this->db->where('hm_epresalepayment.resale_code',$branchid);
