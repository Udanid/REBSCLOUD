...
class Lotdata_model extends CI_Model {
	...
	function get_project_lotdata_id($code) { //get all stock
		$this->db->select('re_prjaclotdata.*,re_prjacblockplane.plan_no,hm_config_designtype.design_name');
		$this->db->where('lot_id',$code);
		$this->db->join('re_prjacblockplane','re_prjacblockplane.prj_id=re_prjaclotdata.prj_id and  re_prjacblockplane.plan_sq=re_prjaclotdata.plan_sqid');
	
	$this->db->join('hm_config_designtype','hm_config_designtype.design_id=re_prjaclotdata.design_id','left');
		$this->db->order_by('CAST(re_prjaclotdata.lot_number AS SIGNED INTEGER)');
	...
	function add_blockoutplan_upload($plan)
	{
		//$tot=$bprice*$quontity; getmaincode($idfield,$prifix,$table)
		$id=$this->getmaincode('plan_sq', $this->input->post('prj_id_upload'),'re_prjacblockplane');
		$data=array( 
		'plan_sq'=>$id,
		'prj_id' => $this->input->post('prj_id_upload'),
		'plan_no' => $this->input->post('plan_no'),
		'drown_by' => $this->input->post('drown_by'),
		'drown_date' => $this->input->post('drown_date_upload'),
		'document ' => $plan,
	...
	//	echo $newsalval;
	$discount=0;
		if(floatval($current_res->discount)>0 & floatval($current_res->discount)<90)
		$discount=$newsalval*$current_res->discount/100;
		if(floatval($current_res->discount)>90)
		{
		$discount=($current_res->discount/$current_lot)*$new_ex;
		} 
 ...
 function getmaincode($idfield,$prifix,$table)
	...
	function if_loan_excist($rescode)
	{
	 ...
	 function update_lotdata($lot_id,$prj_id,$plan_sqid,$lot_number,$extend_perch,$price_perch,$sale_val)
	{
		$data=array(
		'prj_id' =>$prj_id,
		'plan_sqid' =>$plan_sqid,
	...
	function delete_pendinglot_lotid($id)
	{
		//$tot=$bprice*$quontity;
		if($id)
		{
	...
	function onetime_cost_adjestmant($prj_id)
	{
		$this->db->select('re_unrealized.full_cost,re_resevation.lot_id');
		$this->db->join('re_prjaclotdata','re_prjaclotdata.lot_id=re_resevation.lot_id');
		...
		if ($query->num_rows() > 0){
		$datase= $query->result();
			foreach($datase as $raw)
					{
						$data=array(
					'costof_sale' => $raw->full_cost,
					'status' =>'SOLD',
	...
	function delete_blockout($prj_id)
	{
		if($prj_id)
		{
			$this->db->where('prj_id',$prj_id);
			$this->db->delete('re_prjacblockplane');
			$this->db->where('prj_id',$prj_id);
			$this->db->delete('re_prjaclotdata');
		}
		
	}

	//Ticket No:3331 Added By Madushan 2021-08-24
    function get_project_reserved_lots_byprjid($code) { //get all stock
		$this->db->select('re_prjaclotdata.*,re_prjacblockplane.plan_no');
		$this->db->where('re_prjaclotdata.prj_id',$code);
		$this->db->join('re_prjacblockplane','re_prjacblockplane.prj_id=re_prjaclotdata.prj_id and  re_prjacblockplane.plan_sq=re_prjaclotdata.plan_sqid');
	...
	function check_available_blocks($prj_id){
		$this->db->select('*');
		$this->db->where('prj_id',$prj_id);
		$query = $this->db->get('re_prjaclotdata');
		if($query->num_rows>0)
