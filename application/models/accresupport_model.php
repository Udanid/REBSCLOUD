...
class accresupport_model extends CI_Model {
	...
	function get_eploan_data($rescode) { //get all stock //updated by nadee 2747
		$this->db->select('re_eploan.*,re_resevation.prj_id,re_resevation.cus_code2,
    re_resevation.pay_type,re_resevation.discounted_price,re_resevation.profit_status,cm_customerms.first_name,cm_customerms.last_name,cm_customerms.id_number,re_projectms.project_name,re_prjaclotdata.lot_number,re_prjaclotdata.plan_sqid');
...
function get_entry_projectlist($entryid)
{
  ...
  $this->db->join('re_prjacpaymentdata','re_prjacpaymentdata.voucherid=ac_payvoucherdata.voucherid');
...
function get_excess_byincomeid($incomeid)
    {
        $options = array();
        $options[0] = "(Please Select)";
        $this->db->from('re_arreaspayment')->where('pay_id',$incomeid);
        $ledger_q = $this->db->get();
        if ($ledger_q->num_rows() > 0){
            $data= $ledger_q->row();
			return $data->amount;
...
//Ticket No-2774 | Added By Uvini
    function get_excess_byrescode($res_code)
    {
        $options = array();
        $options[0] = "(Please Select)";
        $this->db->from('re_arreaspayment')->where('res_code',$res_code);
        $ledger_q = $this->db->get();
        if ($ledger_q->num_rows() > 0){
            $data= $ledger_q->row();
			return $data->amount;
 ...
 function get_payment_mode($entrynumber) { //get all stock
		$this->db->select('rcvmode');
		$this->db->where('entryid',$entrynumber);
		//$this->db->limit($pagination_counter, $page_count);
		$query = $this->db->get('ac_trnreceipts');
		return $query->row()->rcvmode;
    }
}
