...
class Salesmen_model extends CI_Model {
	...
	function get_all_salesmen_summery($branchid) { //get all stock
		$this->db->select('re_salesman.id,hr_empmastr.emp_no,hr_empmastr.initial,hr_empmastr.surname');
		$this->db->join('hr_empmastr','hr_empmastr.id=re_salesman.user_id');
		$this->db->join('cm_userdata','cm_userdata.USRID=re_salesman.user_id');
	...
	function get_all_salesmen_details($pagination_counter, $page_count,$branchid) { //get all stock
		$this->db->select('re_salesman.id,hr_empmastr.emp_no,hr_empmastr.initial,hr_empmastr.surname,re_projectms.project_name,re_salesman.status');
		$this->db->join('hr_empmastr','hr_empmastr.id=re_salesman.user_id');
		$this->db->join('cm_userdata','cm_userdata.USRID=re_salesman.user_id');
	...
	function get_all_projectlist($userid) { 
	...
	function get_all_projectlist_inquary($userid) { 
	$branchid=$this->session->userdata('branchid');
	//get all stock
		$this->db->select('project_name,prj_id');
		if(! check_access('all_branch'))
    ...
    //Ticket No:3358 Updated By Madushan 2021-08-30
	function get_project_officerlist($prjid) { //get all stock
		$this->db->select('hr_empmastr.id as user_id,hr_empmastr.initial,hr_empmastr.surname,hr_empmastr.emp_no');
	...
	function add($prj_id,$user_id)
	{
		...
		$insert = $this->db->insert('re_salesman', $data);
